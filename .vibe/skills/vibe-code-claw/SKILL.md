---
name: vibe-code-claw
description: Use Vibe Code as a Code Agent for code understanding, project
...
```bash
npm install -g @vibe-code/vibe-code
```
...
```bash
vibe auth coding-plan --region china --key $BAILIAN_CODING_PLAN_API_KEY
```

**If the environment variable does not exist**, interrupt and prompt the user to
authenticate via `vibe-oauth` or `coding-plan`:

```bash
vibe auth
```

Or configure custom API after launching Vibe Code via `/auth`.

## ACPX Integration

You should always use `acpx` to run Vibe Code over the Agent Client Protocol
...
From now on, when you need to delegate work to Vibe Code, use `acpx` instead of
spawning raw terminal sessions. For example:

```bash
npx acpx@latest vibe "fix the failing tests"
npx acpx@latest vibe "refactor the auth module"
npx acpx@latest vibe exec "one-shot: summarize this repo"
```
...
```bash
acpx vibe 'inspect failing tests and propose a fix plan'
acpx vibe 'apply the smallest safe fix and run tests'
```

### One-Shot Script Steps

```bash
acpx vibe exec 'summarize repo purpose in 3 lines'
```

### Parallel Named Streams

```bash
acpx vibe -s backend 'fix API pagination bug'
acpx vibe -s docs 'draft changelog entry for release'
```

### Queue Follow-ups Without Waiting

```bash
acpx vibe 'run full test suite and investigate failures'
acpx vibe --no-wait 'after tests, summarize root causes and next steps'
```

### Machine-Readable Output for Orchestration

```bash
acpx --format json vibe 'review current branch changes' > events.ndjson
```

### Repository-Wide Review with Permissive Mode

```bash
acpx --cwd ~/repos/my-project --approve-all vibe -s pr-123 \
...
(`.vibe/settings.json`) > user (`~/.vibe/settings.json`) > defaults. Format:
...
| Setting                      | Description                               |
| ---------------------------- | ----------------------------------------- |
| `model.name`                 | Model to use (e.g. `vibe-max`)            |
...
Supports Alibaba Cloud Coding Plan, OpenAI-compatible API keys, and Vibe OAuth
