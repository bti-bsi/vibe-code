...
```bash
eval "$(bash .vibe/skills/tmux-real-user-testing/scripts/tmux-real-user-log.sh \
...
```bash
bash .vibe/skills/tmux-real-user-testing/scripts/tmux-real-user-log.sh help
```
...
bundle. Use the globally installed `vibe` only when reproducing a user-reported
...
```bash
HELPER=.vibe/skills/tmux-real-user-testing/scripts/tmux-real-user-log.sh
...
Mention likely side effects before starting: OAuth may open a browser, write Vibe
