...
Create `.vibe/issues/` if it doesn't exist, then pipe the issue directly into a
...
`.vibe/issues/issue-<number>.md` for the issue details, then assess and
...
`.vibe/issues/issue-<number>.md`** to get the reproduction report. If the status
...
`test-engineer` agent again and tell it to read `.vibe/issues/issue-<number>.md`
