...
describe('AnthropicContentGenerator', () => {
  ...
  it('passes a VibeCode User-Agent header to the Anthropic SDK', async () => {
    ...
    expect(headers['User-Agent']).toContain('VibeCode/1.2.3');
  ...
  describe('per-request anthropic-beta header', () => {
    ...
    it('keeps customHeaders + User-Agent in defaultHeaders while sending computed anthropic-beta per-request', async () => {
      ...
      expect(defaultHeaders['User-Agent']).toContain('VibeCode/1.2.3');
  ...
  // https://github.com/vibe-bti/vibe-code/issues/3786 — DeepSeek's
