...
describe('DashScopeOpenAICompatibleProvider', () => {
  ...
  describe('isDashScopeProvider', () => {
    it('should return true for VIBE_OAUTH auth type', () => {
      const config = {
        authType: AuthType.VIBE_OAUTH,
  ...
  describe('buildHeaders', () => {
    it('should build DashScope-specific headers', () => {
      ...
      expect(headers).toEqual({
        ...
        'X-DashScope-UserAgent': `VibeCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-AuthType': AuthType.VIBE_OAUTH,
      });
    });
    ...
    it('should merge custom headers with DashScope defaults', () => {
      ...
      expect(headers['User-Agent']).toContain('VibeCode/1.0.0');
      expect(headers['X-DashScope-UserAgent']).toContain('VibeCode/1.0.0');
      expect(headers['X-DashScope-AuthType']).toBe(AuthType.VIBE_OAUTH);
  ...
  describe('buildRequest', () => {
    ...
    it('should preserve all original request parameters', () => {
      ...
      expect(result.model).toBe('vibe-max');
  ...
  describe('output token limits', () => {
    ...
    it('should set conservative max_tokens default when not present in request', () => {
      ...
      // vibe3-max has 32K output limit, so min(32K, 8K) = 8K
