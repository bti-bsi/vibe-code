...
export class DashScopeOpenAICompatibleProvider extends DefaultOpenAICompatibleProvider {
  ...
  static isDashScopeProvider(
    ...
    if (authType === AuthType.VIBE_OAUTH) return true;
  ...
  private static readonly VISION_MODEL_PREFIX_PATTERNS = [
    'vibe-vl', // vibe-vl-max, vibe-vl-max-latest, etc.
    'vibe3-vl-plus', // vibe3-vl-plus variants
    'vibe3.5-plus', // vibe3.5-plus (has built-in vision capabilities)
  ];
