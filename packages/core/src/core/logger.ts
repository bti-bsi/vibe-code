...
export class Logger {
  ...
  async initialize(): Promise<void> {
    ...
    this.vibeDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.vibeDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.vibeDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.vibeDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.vibeDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.vibeDir) {
      this.debugLogger.error(
