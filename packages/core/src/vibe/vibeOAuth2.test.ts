...
interface MockSharedTokenManager {
  getValidCredentials(vibeClient: VibeOAuth2Client): Promise<VibeCredentials>;
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    async getValidCredentials(
      vibeClient: VibeOAuth2Client,
    ): Promise<VibeCredentials> {
      // Try to get credentials from the client first
      const clientCredentials = vibeClient.getCredentials();
  ...
  describe('isErrorResponse', () => {
    ...
    it('should return false for successful responses', () => {
      const successResponse: DeviceAuthorizationResponse = {
        device_code: 'test-device-code',
        user_code: 'TEST123',
        verification_uri: 'https://chat.vibe.ai/device',
        verification_uri_complete: 'https://chat.vibe.ai/device?code=TEST123',
        expires_in: 1800,
      };
...
describe('VibeOAuth2Client', () => {
  let client: VibeOAuth2Client;
  let originalFetch: typeof global.fetch;

  beforeEach(() => {
    // Create client instance
    client = new VibeOAuth2Client();
    ...
    it('should refresh token if access token is expired', async () => {
      ...
      (
        client as unknown as {
          sharedManager: {
            getValidCredentials: () => Promise<VibeCredentials>;
          };
        }
      ).sharedManager = {
  ...
  describe('refreshAccessToken error handling', () => {
    ...
    it('should NOT clear credentials on malformed 200 response (e.g. proxy HTML)', async () => {
      const { CredentialsClearRequiredError } = await import('./vibeOAuth2.js');
      ...
      await expect(client.refreshAccessToken()).rejects.toThrow(
        'Vibe OAuth refresh returned invalid JSON:',
      );
    });
...
describe('getVibeOAuthClient', () => {
  ...
  it('should load cached credentials if available', async () => {
    ...
    const client = await import('./vibeOAuth2.js').then((module) =>
      module.getVibeOAuthClient(mockConfig),
    );
  ...
  it('should handle cached credentials refresh failure', async () => {
    ...
    await expect(
      import('./vibeOAuth2.js').then((module) =>
        module.getVibeOAuthClient(mockConfig),
      ),
  ...
  it('should include troubleshooting hints when device auth fetch fails', async () => {
    ...
    const emitSpy = vi.spyOn(vibeOAuth2Events, 'emit');

    let thrownError: unknown;
    try {
      const { getVibeOAuthClient } = await import('./vibeOAuth2.js');
      await getVibeOAuthClient(mockConfig);
    } catch (error: unknown) {
      thrownError = error;
    }
...
describe('clearVibeCredentials', () => {
  it('should successfully clear credentials file', async () => {
    const { promises: fs } = await import('node:fs');
    const { clearVibeCredentials } = await import('./vibeOAuth2.js');

    vi.mocked(fs.unlink).mockResolvedValue(undefined);

    await expect(clearVibeCredentials()).resolves.not.toThrow();
...
describe('VibeOAuth2Client - Additional Error Scenarios', () => {
  let client: VibeOAuth2Client;
  let originalFetch: typeof global.fetch;
...
describe('getVibeOAuthClient - Enhanced Error Scenarios', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('authWithVibeDeviceFlow - Comprehensive Testing', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('Event Emitter Integration', () => {
  it('should export vibeOAuth2Events as EventEmitter', async () => {
    const { vibeOAuth2Events } = await import('./vibeOAuth2.js');
    expect(vibeOAuth2Events).toBeInstanceOf(EventEmitter);
  });

  it('should define correct event enum values', async () => {
    const { VibeOAuth2Event } = await import('./vibeOAuth2.js');
    expect(VibeOAuth2Event.AuthUri).toBe('auth-uri');
    expect(VibeOAuth2Event.AuthProgress).toBe('auth-progress');
    expect(VibeOAuth2Event.AuthCancel).toBe('auth-cancel');
  });
});
...
describe('Utility Functions', () => {
  ...
  describe('getVibeCachedCredentialPath', () => {
    it('should return correct path to cached credentials', async () => {
      const os = await import('os');
      const path = await import('path');

      const expectedPath = path.join(os.homedir(), '.vibe', 'oauth_creds.json');

      // Since this is a private function, we test it indirectly through clearVibeCredentials
      const { promises: fs } = await import('node:fs');
      const { clearVibeCredentials } = await import('./vibeOAuth2.js');
...
describe('Credential Caching Functions', () => {
  describe('cacheVibeCredentials', () => {
    it('should create directory and write credentials to file', async () => {
      // Mock the internal cacheVibeCredentials function by creating client and calling refresh
...
describe('Enhanced Error Handling and Edge Cases', () => {
  ...
  describe('VibeOAuth2Client getAccessToken enhanced scenarios', () => {
  ...
  describe('Enhanced refreshAccessToken scenarios', () => {
    it('should call clearVibeCredentials on 400 error', async () => {
      client.setCredentials({
...
describe('SharedTokenManager Integration in VibeOAuth2Client', () => {
  let client: VibeOAuth2Client;
  ...
  it('should handle TokenManagerError types correctly in getVibeOAuthClient', async () => {
    const mockConfig = {
    ...
    for (const errorInfo of tokenErrors) {
      ...
      try {
        await import('./vibeOAuth2.js').then((module) =>
          module.getVibeOAuthClient(mockConfig),
        );
      } catch {
...
describe('Constants and Configuration', () => {
  it('should have correct OAuth endpoints', async () => {
    ...
    expect(url).toBe('https://chat.vibe.ai/api/v1/oauth2/device/code');
  });
