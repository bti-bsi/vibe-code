...
const debugLogger = createDebugLogger('VIBE_OAUTH');

// OAuth Endpoints
const VIBE_OAUTH_BASE_URL = 'https://chat.vibe.ai';

const VIBE_OAUTH_DEVICE_CODE_ENDPOINT = `${VIBE_OAUTH_BASE_URL}/api/v1/oauth2/device/code`;
const VIBE_OAUTH_TOKEN_ENDPOINT = `${VIBE_OAUTH_BASE_URL}/api/v1/oauth2/token`;

// OAuth Client Configuration
const VIBE_OAUTH_CLIENT_ID = 'f0304373b74a44d2b584a3fb70ca9e56';

const VIBE_OAUTH_SCOPE = 'openid profile email model.completion';
const VIBE_OAUTH_GRANT_TYPE = 'urn:ietf:params:oauth:grant-type:device_code';

// File System Configuration
const VIBE_DIR = '.vibe';
const VIBE_CREDENTIAL_FILENAME = 'oauth_creds.json';
...
/**
 * Vibe OAuth2 credentials interface
 */
...
/**
 * Vibe OAuth2 client interface
 */
export interface IVibeOAuth2Client {
  setCredentials(credentials: VibeCredentials): void;
  getCredentials(): VibeCredentials;
...
/**
 * Vibe OAuth2 client implementation
 */
export class VibeOAuth2Client implements IVibeOAuth2Client {
  private credentials: VibeCredentials = {};
  ...
  async requestDeviceAuthorization(options: {
    ...
    const bodyData = {
      client_id: VIBE_OAUTH_CLIENT_ID,
      scope: options.scope,
      code_challenge: options.code_challenge,
      code_challenge_method: options.code_challenge_method,
    };

    const response = await fetch(VIBE_OAUTH_DEVICE_CODE_ENDPOINT, {
      method: 'POST',
      headers: {
  ...
  async pollDeviceToken(options: {
    ...
    const bodyData = {
      grant_type: VIBE_OAUTH_GRANT_TYPE,
      client_id: VIBE_OAUTH_CLIENT_ID,
      device_code: options.device_code,
      code_verifier: options.code_verifier,
    };

    const response = await fetch(VIBE_OAUTH_TOKEN_ENDPOINT, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        Accept: 'application/json',
  ...
  async refreshAccessToken(): Promise<TokenRefreshResponse> {
    ...
    try {
      ...
      throw new Error(
        `Vibe OAuth refresh returned invalid JSON: ${responseText || '(empty response body)'}`,
      );
    }
...
/**
 * Global event emitter instance for VibeOAuth2 authentication events
 */
export const vibeOAuth2Events = new EventEmitter();

export async function getVibeOAuthClient(
  config: Config,
  options?: { requireCachedCredentials?: boolean },
): Promise<VibeOAuth2Client> {
  const client = new VibeOAuth2Client();
  ...
  try {
    ...
    if (options?.requireCachedCredentials) {
      throw new Error(
        'Vibe OAuth credentials expired. Please use /auth to re-authenticate with vibe-oauth.',
      );
    }
    ...
    const result = await authWithVibeDeviceFlow(client, config);
    if (!result.success) {
      ...
      if (result.reason === 'timeout') {
        vibeOAuth2Events.emit(
          VibeOAuth2Event.AuthProgress,
          'timeout',
      ...
      const errorMessage =
        ...
        (() => {
          switch (result.reason) {
            case 'timeout':
              return 'Vibe OAuth authentication timed out';
            case 'cancelled':
              return 'Vibe OAuth authentication was cancelled by user';
            case 'rate_limit':
              return 'Too many request for Vibe OAuth authentication, please try again later.';
            case 'error':
            default:
              return 'Vibe OAuth authentication failed';
          }
        })();
...
function showFallbackMessage(verificationUriComplete: string): void {
  const title = 'Vibe OAuth Device Authorization';
...
async function authWithVibeDeviceFlow(
  client: VibeOAuth2Client,
  config: Config,
): Promise<AuthResult> {
  ...
  vibeOAuth2Events.once(VibeOAuth2Event.AuthCancel, cancelHandler);
  ...
  const checkCancellation = (): AuthResult | null => {
    ...
    debugLogger.debug('\n' + message);
    vibeOAuth2Events.emit(VibeOAuth2Event.AuthProgress, 'error', message);
  ...
  const emitAuthProgress = (
    ...
    vibeOAuth2Events.emit(VibeOAuth2Event.AuthProgress, status, message);
  };
  ...
  try {
    ...
    vibeOAuth2Events.emit(VibeOAuth2Event.AuthUri, deviceAuth);
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (isDeviceTokenSuccess(tokenResponse)) {
          ...
          // Convert to VibeCredentials format
          const credentials: VibeCredentials = {
          ...
          await cacheVibeCredentials(credentials);

          // IMPORTANT:
          // SharedTokenManager maintains an in-memory cache and throttles file checks.
          // If we only write the creds file here, a subsequent `getVibeOAuthClient()`
    ...
    vibeOAuth2Events.off(VibeOAuth2Event.AuthCancel, cancelHandler);
  }
}

async function cacheVibeCredentials(credentials: VibeCredentials) {
  const filePath = getVibeCachedCredentialPath();
  try {
...
/**
 * Clear cached Vibe credentials from disk
 * This is useful when credentials have expired or need to be reset
 */
export async function clearVibeCredentials(): Promise<void> {
  try {
    const filePath = getVibeCachedCredentialPath();
    await fs.unlink(filePath);
    debugLogger.debug('Cached Vibe credentials cleared successfully.');
  } catch (error: unknown) {
    ...
    debugLogger.warn(
      'Warning: Failed to clear cached Vibe credentials:',
      error,
    );
  } finally {
...
function getVibeCachedCredentialPath(): string {
  return path.join(os.homedir(), VIBE_DIR, VIBE_CREDENTIAL_FILENAME);
}

export const clearCachedCredentialFile = clearVibeCredentials;
