...
const VIBE_LOCK_FILENAME = 'oauth_creds.lock';
...
interface MemoryCache {
  credentials: VibeCredentials | null;
  fileModTime: number;
  lastCheck: number;
}

/**
 * Validates that the given data is a valid VibeCredentials object
 *
 * @param data - The data to validate
 * @returns The validated credentials object
 * @throws Error if the data is invalid
 */
function validateCredentials(data: unknown): VibeCredentials {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid credentials format');
  }

  const creds = data as Partial<VibeCredentials>;
  ...
  return creds as VibeCredentials;
}
...
export class SharedTokenManager {
   ...
   */
  private refreshPromise: Promise<VibeCredentials> | null = null;
   ...
   *
   * @param vibeClient - The OAuth2 client instance
  ...
  async getValidCredentials(
    ...
    try {
      ...
      await this.checkAndReloadIfNeeded(vibeClient);
  ...
  private async checkAndReloadIfNeeded(
    ...
    this.checkPromise = this.performFileCheck(vibeClient, now);
  ...
  private async performFileCheck(
    vibeClient: IVibeOAuth2Client | undefined,
    checkTime: number,
  ): Promise<void> {
    ...
    try {
      ...
      if (fileModTime > this.memoryCache.fileModTime) {
        await this.reloadCredentialsFromFile(vibeClient);
   ...
   */
  private async forceFileCheck(vibeClient?: IVibeOAuth2Client): Promise<void> {
    try {
      const filePath = this.getCredentialFilePath();
  ...
  /**
   * Load credentials from the file system into memory cache and sync with vibeClient
   */
  private async reloadCredentialsFromFile(
    ...
    try {
      ...
      // Sync with vibeClient atomically - rollback on failure
      try {
        if (vibeClient) {
          vibeClient.setCredentials(credentials);
        }
      } catch (clientError) {
  ...
  private async performTokenRefresh(
    ...
    try {
      ...
      const currentCredentials = vibeClient.getCredentials();
      ...
      await this.forceFileCheck(vibeClient);
      ...
      if (
        ...
        // No need to call vibeClient.setCredentials here as checkAndReloadIfNeeded already did it
        return this.memoryCache.credentials;
      }

      // Perform the actual token refresh
      const response = await vibeClient.refreshAccessToken();
   ...
   */
  private isTokenValid(credentials: VibeCredentials): boolean {
  ...
  private getLockFilePath(): string {
    return path.join(os.homedir(), VIBE_DIR, VIBE_LOCK_FILENAME);
  }
