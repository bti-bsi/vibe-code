...
/**
 * Vibe Content Generator that uses Vibe OAuth tokens with automatic refresh
 */
export class VibeContentGenerator extends OpenAIContentGenerator {
  private readonly debugLogger = createDebugLogger('VIBE');
  private vibeClient: IVibeOAuth2Client;
  ...
  constructor(
    ...
    // Create DashScope provider for Vibe
  ...
  private async executeWithCredentialManagement<T>(
    ...
    try {
      ...
      if (this.isAuthError(error)) {
        ...
        await this.sharedManager.getValidCredentials(this.vibeClient, true);
        return await attemptOperation();
      }
      throw error;
    }
