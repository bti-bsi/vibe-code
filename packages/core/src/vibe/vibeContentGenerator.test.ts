...
import type { IVibeOAuth2Client } from './vibeOAuth2.js';
import { type VibeCredentials, type ErrorData } from './vibeOAuth2.js';
import type {
...
import { VibeContentGenerator } from './vibeContentGenerator.js';
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    private mockCredentials: VibeCredentials | null = null;
    ...
    async getValidCredentials(
      vibeClient: IVibeOAuth2Client,
    ): Promise<VibeCredentials> {
      ...
      try {
        const { token } = await vibeClient.getAccessToken();
        if (token) {
          const credentials = vibeClient.getCredentials();
          return credentials;
        }
      } catch (error) {
        ...
        if (isAuthError) {
          ...
          try {
            const refreshResult = await vibeClient.refreshAccessToken();
            if (refreshResult && !('error' in refreshResult)) {
              // Refresh succeeded, update client credentials and return them
              const updatedCredentials = vibeClient.getCredentials();
              return updatedCredentials;
            } else {
              // Refresh failed, throw appropriate error
              throw new Error(
                'Failed to obtain valid Vibe access token. Please re-authenticate.',
              );
            }
          } catch {
            throw new Error(
    ...
    getCurrentCredentials(): VibeCredentials | null {
      return this.mockCredentials;
    }
    ...
    setMockCredentials(credentials: VibeCredentials | null): void {
...
describe('VibeContentGenerator', () => {
  let mockVibeClient: IVibeOAuth2Client;
  let vibeContentGenerator: VibeContentGenerator;
  let mockConfig: Config;

  const mockCredentials: VibeCredentials = {
  ...
  beforeEach(() => {
    ...
    // Create VibeContentGenerator instance
    ...
    vibeContentGenerator = new VibeContentGenerator(
      mockVibeClient,
  ...
  describe('Core Content Generation Methods', () => {
    it('should generate content with valid token', async () => {
      vi.mocked(mockVibeClient.getAccessToken).mockResolvedValue({
        token: 'valid-token',
      });
      vi.mocked(mockVibeClient.getCredentials).mockReturnValue(mockCredentials);
      ...
      const result = await vibeContentGenerator.generateContent(
        request,
        'test-prompt-id',
      );

      expect(result.text).toBe('Generated content');
      expect(mockVibeClient.getAccessToken).toHaveBeenCalled();
    });
    ...
    it('should generate content stream with valid token', async () => {
      ...
      const stream = await vibeContentGenerator.generateContentStream(
        request,
        'test-prompt-id',
      );
      const chunks: string[] = [];
    ...
    it('should count tokens without requiring authentication', async () => {
      ...
      const result = await vibeContentGenerator.countTokens(request);

      expect(result.totalTokens).toBe(15);
      // countTokens is a local operation and should not require OAuth credentials
      expect(mockVibeClient.getAccessToken).not.toHaveBeenCalled();
    });
    ...
    it('should embed content with valid token', async () => {
      ...
      const result = await vibeContentGenerator.embedContent(request);
  ...
  describe('Token Management and Refresh Logic', () => {
    it('should refresh token on auth error and retry', async () => {
      ...
      vi.mocked(mockVibeClient.getAccessToken)
        .mockRejectedValueOnce(authError)
        .mockResolvedValueOnce({ token: 'refreshed-token' });

      // Refresh succeeds
      vi.mocked(mockVibeClient.refreshAccessToken).mockResolvedValue({
      ...
      vi.mocked(mockVibeClient.getCredentials).mockReturnValue({
        access_token: 'refreshed-token',
        token_type: 'Bearer',
      ...
      expect(result.text).toBe('Generated content');
      expect(mockVibeClient.refreshAccessToken).toHaveBeenCalled();
    });
    ...
    it('should handle token refresh failure', async () => {
      ...
      await expect(
        vibeContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow(
        'Failed to obtain valid Vibe access token. Please re-authenticate.',
      );
    ...
    it('should update endpoint when token is refreshed', async () => {
      ...
      await vibeContentGenerator.generateContent(request, 'test-prompt-id');

      expect(mockVibeClient.getCredentials).toHaveBeenCalled();
    });
  });
  ...
  describe('Endpoint URL Normalization', () => {
    it('should use default endpoint when no custom endpoint provided', async () => {
      ...
      const parentPrototype = Object.getPrototypeOf(
        Object.getPrototypeOf(vibeContentGenerator),
      );
  ...
  describe('Client State Management', () => {
    it('should set dynamic credentials during operations', async () => {
      const client = (
        vibeContentGenerator as unknown as {
  ...
  describe('Error Handling and Retry Logic', () => {
    it('should retry once on authentication errors', async () => {
      ...
      vi.mocked(mockVibeClient.getAccessToken).mockImplementation(async () => {
    ...
    it('should not retry non-authentication errors', async () => {
      ...
      expect(mockVibeClient.refreshAccessToken).not.toHaveBeenCalled();
    ...
    it('should handle error response from token refresh', async () => {
      vi.mocked(mockVibeClient.getAccessToken).mockRejectedValue(
      ...
      await expect(
        vibeContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow('Failed to obtain valid Vibe access token');
    });
  });

  describe('Token State Management', () => {
    it('should cache and return current token', () => {
      expect(vibeContentGenerator.getCurrentToken()).toBeNull();

      // Simulate setting a token internally
      (
        vibeContentGenerator as unknown as { currentToken: string }
      ).currentToken = 'cached-token';

      expect(vibeContentGenerator.getCurrentToken()).toBe('cached-token');
    });

    it('should clear token on clearToken()', () => {
      // Simulate having cached token value
      const vibeInstance = vibeContentGenerator as unknown as {
        currentToken: string;
      };
      vibeInstance.currentToken = 'cached-token';

      vibeContentGenerator.clearToken();
    ...
    it('should handle concurrent token refresh requests', async () => {
      ...
      vi.mocked(mockVibeClient.getAccessToken).mockRejectedValue(authError);
      vi.mocked(mockVibeClient.getCredentials).mockReturnValue(mockCredentials);

      vi.mocked(mockVibeClient.refreshAccessToken).mockImplementation(
        async () => {
          refreshCallCount++;
  ...
  describe('SharedTokenManager Integration', () => {
    it('should use SharedTokenManager to get valid credentials', async () => {
      ...
      const newGenerator = new VibeContentGenerator(
        mockVibeClient,
        { model: 'vibe-turbo', authType: AuthType.VIBE_OAUTH },
        mockConfig,
      );
  ...
  describe('getCurrentEndpoint Method', () => {
    it('should handle URLs with custom ports', () => {
      ...
      endpoints.forEach(({ input, expected }) => {
        ...
        const generator = vibeContentGenerator as unknown as {
  ...
  describe('Stream Error Handling', () => {
    it('should set credentials when stream generation fails', async () => {
      ...
      try {
        await vibeContentGenerator.generateContentStream(
          request,
          'test-prompt-id',
        );
      } catch (error) {
  ...
  describe('Constructor and Initialization', () => {
    it('should initialize with configured base URL when provided', () => {
      const generator = new VibeContentGenerator(
        mockVibeClient,
        {
          model: 'vibe-turbo',
          authType: AuthType.VIBE_OAUTH,
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('should handle method types with token failure (except countTokens)', async () => {
      ...
      await expect(newGenerator.embedContent(embedRequest)).rejects.toThrow(
        'Failed to obtain valid Vibe access token',
      );
