...
import { VIBE_OAUTH_MODELS } from './constants.js';
import { createDebugLogger } from '../utils/debugLogger.js';

const debugLogger = createDebugLogger('MODEL_REGISTRY');

export { VIBE_OAUTH_MODELS } from './constants.js';
...
export class ModelRegistry {
  ...
  private getDefaultBaseUrl(authType: AuthType): string {
    switch (authType) {
      case AuthType.VIBE_OAUTH:
        return 'DYNAMIC_VIBE_OAUTH_BASE_URL';
  ...
  constructor(modelProvidersConfig?: ModelProvidersConfig) {
    ...
    // Always register vibe-oauth models (hard-coded, cannot be overridden)
    this.registerAuthTypeModels(AuthType.VIBE_OAUTH, VIBE_OAUTH_MODELS);
    ...
    if (modelProvidersConfig) {
      for (const [rawKey, models] of Object.entries(modelProvidersConfig)) {
        ...
        // Skip vibe-oauth as it uses hard-coded models
   ...
   * For vibe-oauth, returns the coder model.
  ...
  getDefaultModelForAuthType(
    ...
    if (authType === AuthType.VIBE_OAUTH) {
      return this.getModel(authType, DEFAULT_VIBE_MODEL);
    }
    const models = this.modelsByAuthType.get(authType);
   ...
   * Preserves hard-coded vibe-oauth models.
   */
  reloadModels(modelProvidersConfig?: ModelProvidersConfig): void {
    // Clear existing user-configured models (preserve vibe-oauth)
