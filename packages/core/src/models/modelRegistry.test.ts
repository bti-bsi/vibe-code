...
import { ModelRegistry, VIBE_OAUTH_MODELS } from './modelRegistry.js';
import { AuthType } from '../core/contentGenerator.js';
import type { ModelProvidersConfig } from './types.js';

describe('ModelRegistry', () => {
  describe('initialization', () => {
    it('should always include hard-coded vibe-oauth models', () => {
      const registry = new ModelRegistry();

      const vibeModels = registry.getModelsForAuthType(AuthType.VIBE_OAUTH);
      expect(vibeModels.length).toBe(VIBE_OAUTH_MODELS.length);
      expect(vibeModels[0].id).toBe('coder-model');
    });

    it('should initialize with empty config', () => {
      const registry = new ModelRegistry();
      expect(registry.getModelsForAuthType(AuthType.VIBE_OAUTH).length).toBe(
        VIBE_OAUTH_MODELS.length,
      );
    ...
    it('should ignore vibe-oauth models in config (hard-coded)', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        'vibe-oauth': [
          {
            id: 'custom-vibe',
      ...
      // Should still use hard-coded vibe-oauth models
      const vibeModels = registry.getModelsForAuthType(AuthType.VIBE_OAUTH);
      expect(vibeModels.length).toBe(VIBE_OAUTH_MODELS.length);
      expect(vibeModels.find((m) => m.id === 'custom-vibe')).toBeUndefined();
    });
  });
  ...
  describe('getDefaultModelForAuthType', () => {
    it('should return coder-model for vibe-oauth', () => {
      const registry = new ModelRegistry();
  ...
  describe('default base URLs', () => {
    it('should apply default dashscope URL for vibe-oauth', () => {
      const registry = new ModelRegistry();
      const model = registry.getModel(AuthType.VIBE_OAUTH, 'coder-model');
      expect(model?.baseUrl).toBe('DYNAMIC_VIBE_OAUTH_BASE_URL');
    });
  ...
  describe('reloadModels', () => {
    ...
    it('should preserve hard-coded vibe-oauth models after reload', () => {
      const registry = new ModelRegistry({
        openai: [{ id: 'gpt-4', name: 'GPT-4' }],
      });
      ...
      expect(
        registry.getModel(AuthType.VIBE_OAUTH, 'coder-model'),
      ).toBeDefined();
    });
    ...
    it('should ignore vibe-oauth models in reload config', () => {
      const registry = new ModelRegistry();

      registry.reloadModels({
        'vibe-oauth': [{ id: 'custom-vibe', name: 'Custom Vibe' }],
      });

      // vibe-oauth should still use hard-coded models
      const vibeModels = registry.getModelsForAuthType(AuthType.VIBE_OAUTH);
      expect(vibeModels.length).toBe(VIBE_OAUTH_MODELS.length);
      expect(vibeModels.find((m) => m.id === 'custom-vibe')).toBeUndefined();
    });
