...
describe('GlobTool', () => {
  ...
  describe('ignore file handling', () => {
    ...
    it('should respect .vibeignore files by default', async () => {
      await fs.writeFile(
        path.join(tempRootDir, '.vibeignore'),
        '*.vibeignored.txt',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'a.vibeignored.txt'),
        'ignored content',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'b.notignored.txt'),
      ...
      // Recreate the tool to pick up the new .vibeignore file
      globTool = new GlobTool(mockConfig);
      ...
      expect(result.llmContent).toContain('Found 3 file(s)'); // fileA.txt, FileB.TXT, b.notignored.txt
      expect(result.llmContent).not.toContain('a.vibeignored.txt');
    });
    ...
    it('should respect .vibeignore when searching a subdirectory (path option)', async () => {
      await fs.writeFile(path.join(tempRootDir, '.vibeignore'), '*.secret');
      await fs.writeFile(path.join(tempRootDir, 'sub', 'visible.txt'), 'ok');
      await fs.writeFile(
        path.join(tempRootDir, 'sub', 'hidden.secret'),
        'should be ignored',
      ...
      // Recreate to pick up .vibeignore
      const subDirTool = new GlobTool(mockConfig);
      const params: GlobToolParams = { pattern: '*', path: 'sub' };
      const invocation = subDirTool.build(params);
      const result = await invocation.execute(abortSignal);
