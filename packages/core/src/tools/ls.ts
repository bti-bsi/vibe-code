...
export interface LSToolParams {
  ...
  /**
   * Whether to respect .gitignore and .vibeignore patterns (optional, defaults to true)
   */
  file_filtering_options?: {
    respect_git_ignore?: boolean;
    respect_vibe_ignore?: boolean;
  };
}
...
class LSToolInvocation extends BaseToolInvocation<LSToolParams, ToolResult> {
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      const { filteredPaths, gitIgnoredCount, vibeIgnoredCount } =
        fileDiscovery.filterFilesWithReport(relativePaths, {
          ...
          respectVibeIgnore:
            this.params.file_filtering_options?.respect_vibe_ignore ??
            this.config.getFileFilteringOptions().respectVibeIgnore ??
      ...
      if (vibeIgnoredCount > 0) {
        ignoredMessages.push(`${vibeIgnoredCount} vibe-ignored`);
      }
...
- file_filtering_options (object): Optional: Whether to respect ignore patterns from .gitignore or .vibeignore`,
      Kind.Search,
      {
        properties: {
          ...
          file_filtering_options: {
            description:
              'Optional: Whether to respect ignore patterns from .gitignore or .vibeignore',
            type: 'object',
            properties: {
              respect_git_ignore: {
                description:
              ...
              respect_vibe_ignore: {
                description:
                  'Optional: Whether to respect .vibeignore patterns when listing files. Defaults to true.',
                type: 'boolean',
              },
            },
          },
