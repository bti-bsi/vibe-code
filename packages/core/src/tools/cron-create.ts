    ...
    try {
      ...
      if (recurring) {
        llmContent =
          ...
          'Session-only (not written to disk, dies when Vibe Code exits). ' +
  ...
  constructor(private config: Config) {
    super(
      ...
      'Schedule a prompt to be enqueued at a future time. Use for both recurring schedules and one-shot reminders.\n\n' +
              ...
              '## Session-only\n\n' +
              'Jobs live only in this Vibe Code session — nothing is written to disk, and the job is gone when Vibe Code exits.\n\n' +
              '## Runtime behavior\n\n' +
...
- recurring (boolean): true (default) = fire on every cron match until deleted or auto-expired after 3 days. false = fire once at the next match, then auto-delete. Use false for "remind me at X" one-shot requests with pinned minute/hour/dom/month.`,
      Kind.Other,
      {
        type: 'object',
        properties: {
