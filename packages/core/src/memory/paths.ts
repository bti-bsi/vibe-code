...
import { VIBE_DIR, sanitizeCwd } from '../utils/paths.js';
 ...
 * Defaults to `~/.vibe`; overridable via VIBE_CODE_MEMORY_BASE_DIR for tests.
 */
export function getMemoryBaseDir(): string {
  if (process.env['VIBE_CODE_MEMORY_BASE_DIR']) {
    return process.env['VIBE_CODE_MEMORY_BASE_DIR'];
  }
  return path.join(os.homedir(), VIBE_DIR);
}
...
export function getAutoMemoryRoot(projectRoot: string): string {
  ...
  let result: string;
  if (process.env['VIBE_CODE_MEMORY_LOCAL'] === '1') {
    result = path.join(projectRoot, VIBE_DIR, AUTO_MEMORY_DIRNAME);
  } else {
    const canonicalRoot =
