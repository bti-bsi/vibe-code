 ...
 * out at runtime: `VIBE_DISABLE_AUTO_TITLE=1` (or any truthy-ish value)
...
function autoTitleDisabledByEnv(): boolean {
  const v = process.env['VIBE_DISABLE_AUTO_TITLE'];
  if (!v) return false;
 ...
 *
 * File location: ~/.vibe/tmp/<project_id>/chats/
 *
...
export class ChatRecordingService {
  ...
  private maybeTriggerAutoTitle(): void {
    ...
    // Headless/one-shot CLI flows (`vibe -p "…"`, cron, CI scripts) run a
