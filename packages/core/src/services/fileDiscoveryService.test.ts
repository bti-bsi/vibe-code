...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .vibeignore patterns even when not in a git repo', async () => {
      await createTestFile('.vibeignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldVibeIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldVibeIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.vibeignore', 'logs/');
    });

    it('should filter out git-ignored and vibe-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectVibeIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectVibeIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldVibeIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.vibeignore', '*.log');
    });
    ...
    it('should return true for vibe-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldVibeIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });

    it('should return false for non-vibe-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldVibeIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
