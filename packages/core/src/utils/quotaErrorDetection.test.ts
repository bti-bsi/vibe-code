...
describe('quotaErrorDetection', () => {
  describe('isVibeQuotaExceededError', () => {
    it('should detect the Vibe insufficient_quota error', () => {
      const error = {
        status: 429,
        code: 'insufficient_quota',
        message: 'Free allocated quota exceeded.',
      };
      expect(isVibeQuotaExceededError(error)).toBe(true);
    });
    ...
    it('should not match when status is not 429', () => {
      ...
      expect(isVibeQuotaExceededError(error)).toBe(false);
    });
    ...
    it('should not match string errors', () => {
      expect(isVibeQuotaExceededError('insufficient_quota')).toBe(false);
    });

    it('should not match null or undefined', () => {
      expect(isVibeQuotaExceededError(null)).toBe(false);
      expect(isVibeQuotaExceededError(undefined)).toBe(false);
    });
  });
