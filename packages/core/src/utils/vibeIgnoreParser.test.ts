...
import { VibeIgnoreParser } from './vibeIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';

describe('VibeIgnoreParser', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'vibeignore-test-'));
  });
  ...
  describe('when .vibeignore exists', () => {
    beforeEach(async () => {
    ...
    it('should ignore files specified in .vibeignore', () => {
      const parser = new VibeIgnoreParser(projectRoot);
  ...
  describe('when .vibeignore does not exist', () => {
