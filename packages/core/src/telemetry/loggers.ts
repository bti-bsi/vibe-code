...
import { VibeLogger } from './vibe-logger/vibe-logger.js';
...
export function logStartSession(
  ...
  VibeLogger.getInstance(config)?.logStartSessionEvent(event);
...
export function logUserPrompt(config: Config, event: UserPromptEvent): void {
  VibeLogger.getInstance(config)?.logNewPromptEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserRetry(config: Config, event: UserRetryEvent): void {
  VibeLogger.getInstance(config)?.logRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolCall(config: Config, event: ToolCallEvent): void {
  ...
  VibeLogger.getInstance(config)?.logToolCallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolOutputTruncated(
  ...
  VibeLogger.getInstance(config)?.logToolOutputTruncatedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFileOperation(
  ...
  VibeLogger.getInstance(config)?.logFileOperationEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiRequest(config: Config, event: ApiRequestEvent): void {
  // VibeLogger.getInstance(config)?.logApiRequestEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFlashFallback(
  ...
  VibeLogger.getInstance(config)?.logFlashFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logRipgrepFallback(
  ...
  VibeLogger.getInstance(config)?.logRipgrepFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiError(config: Config, event: ApiErrorEvent): void {
  ...
  VibeLogger.getInstance(config)?.logApiErrorEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiCancel(config: Config, event: ApiCancelEvent): void {
  ...
  uiTelemetryService.addEvent(uiEvent);
  VibeLogger.getInstance(config)?.logApiCancelEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiResponse(config: Config, event: ApiResponseEvent): void {
  ...
  VibeLogger.getInstance(config)?.logApiResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logLoopDetected(
  ...
  VibeLogger.getInstance(config)?.logLoopDetectedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logLoopDetectionDisabled(
  ...
  VibeLogger.getInstance(config)?.logLoopDetectionDisabledEvent();
}
...
export function logNextSpeakerCheck(
  ...
  VibeLogger.getInstance(config)?.logNextSpeakerCheck(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSlashCommand(
  ...
  VibeLogger.getInstance(config)?.logSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logIdeConnection(
  ...
  VibeLogger.getInstance(config)?.logIdeConnectionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logConversationFinishedEvent(
  ...
  VibeLogger.getInstance(config)?.logConversationFinishedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logChatCompression(
  ...
  VibeLogger.getInstance(config)?.logChatCompressionEvent(event);
...
export function logKittySequenceOverflow(
  ...
  VibeLogger.getInstance(config)?.logKittySequenceOverflowEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logMalformedJsonResponse(
  ...
  VibeLogger.getInstance(config)?.logMalformedJsonResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logInvalidChunk(
  ...
  VibeLogger.getInstance(config)?.logInvalidChunkEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetry(
  ...
  VibeLogger.getInstance(config)?.logContentRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetryFailure(
  ...
  VibeLogger.getInstance(config)?.logContentRetryFailureEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSubagentExecution(
  ...
  VibeLogger.getInstance(config)?.logSubagentExecutionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logModelSlashCommand(
  ...
  VibeLogger.getInstance(config)?.logModelSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logHookCall(config: Config, event: HookCallEvent): void {
  // Log to VibeLogger for RUM telemetry only
  VibeLogger.getInstance(config)?.logHookCallEvent(event);
}
...
export function logExtensionInstallEvent(
  ...
  VibeLogger.getInstance(config)?.logExtensionInstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionUninstall(
  ...
  VibeLogger.getInstance(config)?.logExtensionUninstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export async function logExtensionUpdateEvent(
  ...
  VibeLogger.getInstance(config)?.logExtensionUpdateEvent(event);
...
export function logExtensionEnable(
  ...
  VibeLogger.getInstance(config)?.logExtensionEnableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionDisable(
  ...
  VibeLogger.getInstance(config)?.logExtensionDisableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logAuth(config: Config, event: AuthEvent): void {
  VibeLogger.getInstance(config)?.logAuthEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserFeedback(
  ...
  uiTelemetryService.addEvent(uiEvent);
  config.getChatRecordingService()?.recordUiTelemetryEvent(uiEvent);
  VibeLogger.getInstance(config)?.logUserFeedbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionStarted(
  ...
  VibeLogger.getInstance(config)?.logArenaSessionStartedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaAgentCompleted(
  ...
  VibeLogger.getInstance(config)?.logArenaAgentCompletedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionEnded(
  ...
  VibeLogger.getInstance(config)?.logArenaSessionEndedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
