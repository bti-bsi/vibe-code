...
export class VibeLogger {
  ...
  private constructor(config: Config) {
    this.config = config;
    this.debugLogger = createDebugLogger('VIBE_LOGGER');
  ...
  static getInstance(config?: Config): VibeLogger | undefined {
    if (config === undefined || !config?.getUsageStatisticsEnabled())
      return undefined;
    if (!VibeLogger.instance) {
      VibeLogger.instance = new VibeLogger(config);
    }
    ...
    try {
      ...
      if (wasAtCapacity) {
        this.debugLogger.debug(
          `VibeLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
      this.debugLogger.error('VibeLogger: Failed to enqueue log event.', error);
    }
  }
  ...
  readSourceInfo(): string {
    try {
      const sourceJsonPath = path.join(os.homedir(), '.vibe', 'source.json');
  ...
  async flushToRum(): Promise<LogResponse> {
    if (this.isFlushInProgress) {
      this.debugLogger.debug(
        'VibeLogger: Flush already in progress, marking pending flush.',
      );
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    this.debugLogger.debug(
      `VibeLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
    );
  }
}
