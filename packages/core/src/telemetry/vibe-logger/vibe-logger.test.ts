...
import { VibeLogger, TEST_ONLY } from './vibe-logger.js';
...
const makeFakeConfig = (overrides: Partial<Config> = {}): Config => {
  const defaults = {
    ...
    getAuthType: () => AuthType.VIBE_OAUTH,
    getMcpServers: () => ({}),
    getModel: () => 'test-model',
...
describe('VibeLogger', () => {
  ...
  beforeEach(() => {
    ...
    (VibeLogger as any).instance = undefined;
  });
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = VibeLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = VibeLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(VibeLogger);
    });

    it('is a singleton', () => {
      const logger1 = VibeLogger.getInstance(mockConfig);
      const logger2 = VibeLogger.getInstance(mockConfig);
  ...
  describe('logHookCallEvent', () => {
    ...
    it('should log a failed hook call event with error when telemetry log prompts enabled', () => {
      ...
      const logger = VibeLogger.getInstance(configWithLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should not include error when telemetry log prompts disabled', () => {
      ...
      (VibeLogger as unknown as { instance: undefined }).instance = undefined;
      const logger = VibeLogger.getInstance(configWithoutLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should sanitize hook name to remove sensitive information', () => {
      ...
      const event = new HookCallEvent(
        'PreToolUse',
        'command',
        '/home/user/.vibe/hooks/check-secrets.sh --api-key=secret123',
        { tool_name: 'read_file' },
        100,
        true,
      );
