...
import { VibeConnectionHandler } from './vibeConnectionHandler.js';
import type { AcpConnection } from './acpConnection.js';

describe('VibeConnectionHandler', () => {
  let handler: VibeConnectionHandler;
  ...
  beforeEach(async () => {
    ...
    handler = new VibeConnectionHandler();
    mockConnection = {
    ...
    it('retries connect on spawn failure and succeeds on second attempt', async () => {
      (mockConnection.connect as ReturnType<typeof vi.fn>)
        .mockRejectedValueOnce(
          new Error(
            'Vibe ACP process failed to start (exit code: null, signal: SIGTERM)',
          ),
        )
