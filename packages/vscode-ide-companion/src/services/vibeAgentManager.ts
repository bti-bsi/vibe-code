...
import { VibeSessionReader, type VibeSession } from './vibeSessionReader.js';
import { VibeSessionManager } from './vibeSessionManager.js';
import type {
  ChatMessage,
  PlanEntry,
  ToolCallUpdateData,
...
import {
  VibeConnectionHandler,
  type VibeConnectionResult,
} from '../services/vibeConnectionHandler.js';
import { VibeSessionUpdateHandler } from './vibeSessionUpdateHandler.js';
...
export class VibeAgentManager {
  private connection: AcpConnection;
  private sessionReader: VibeSessionReader;
  private sessionManager: VibeSessionManager;
  private connectionHandler: VibeConnectionHandler;
  private sessionUpdateHandler: VibeSessionUpdateHandler;
  ...
  // Callback storage
  private callbacks: VibeAgentCallbacks = {};
  ...
  constructor() {
    ...
    this.sessionReader = new VibeSessionReader();
    this.sessionManager = new VibeSessionManager();
    this.connectionHandler = new VibeConnectionHandler();
    this.sessionUpdateHandler = new VibeSessionUpdateHandler({});
    ...
    this.connection.onSessionUpdate = (data: SessionNotification) => {
      ...
      try {
        ...
        console.warn('[VibeAgentManager] Rehydration routing failed:', err);
      }
    ...
    this.connection.onEndTurn = (reason?: string) => {
      try {
        ...
        console.warn('[VibeAgentManager] onEndTurn callback error:', err);
      }
    };
    ...
    this.connection.onAuthenticateUpdate = (
      ...
      try {
        ...
        console.warn(
          '[VibeAgentManager] onAuthenticateUpdate callback error:',
          err,
        );
      }
    };
    ...
    this.connection.onInitialized = (init: unknown) => {
      try {
        ...
        console.warn('[VibeAgentManager] onInitialized parse error:', err);
      }
    };
    ...
    this.connection.onDisconnected = (
      ...
      console.log(
        `[VibeAgentManager] Process disconnected (code: ${code}, signal: ${signal})`,
      );
  ...
  async connect(
    ...
    if (res.availableModels && res.availableModels.length > 0) {
      ...
      console.log(
        '[VibeAgentManager] Emitting availableModels from connect():',
  ...
  async reconnect(
    ...
    console.log('[VibeAgentManager] Attempting reconnection...');
    try {
  ...
  async setApprovalModeFromUi(
    ...
    try {
      ...
      console.error('[VibeAgentManager] Failed to set mode:', err);
      throw err;
    }
  }
  ...
  async setModelFromUi(modelId: string): Promise<ModelInfo | null> {
    try {
      ...
      console.error('[VibeAgentManager] Failed to set model:', err);
      throw err;
    }
  }
  ...
  async validateCurrentSession(): Promise<boolean> {
    try {
      ...
      console.warn('[VibeAgentManager] Session validation failed:', error);
  ...
  async getSessionList(): Promise<Array<Record<string, unknown>>> {
    console.log(
      '[VibeAgentManager] Getting session list with version-aware strategy',
    );

    try {
      console.log(
        '[VibeAgentManager] Attempting to get session list via ACP method',
      );
      const response = await this.connection.listSessions();
      console.log('[VibeAgentManager] ACP session list response:', response);

      const res: unknown = response;
      const items = extractSessionListItems(res);

      console.log(
        '[VibeAgentManager] Sessions retrieved via ACP:',
        res,
        items.length,
      );
      if (items.length > 0) {
      ...
      console.warn(
        '[VibeAgentManager] ACP session list failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log('[VibeAgentManager] Getting session list from file system');
      const sessions = await this.sessionReader.getAllSessions(undefined, true);
      console.log(
        '[VibeAgentManager] Session list from file system (all projects):',
        sessions.length,
      );

      const result = sessions.map(
        (session: VibeSession): Record<string, unknown> => ({
      ...
      console.log(
        '[VibeAgentManager] Sessions retrieved from file system:',
        result.length,
      );
      return result;
    } catch (error) {
      console.error(
        '[VibeAgentManager] Failed to get session list from file system:',
        error,
      );
      return [];
    }
  ...
  async getSessionListPaged(params?: {
    ...
    try {
      ...
      console.warn('[VibeAgentManager] Paged ACP session list failed:', error);
    ...
    try {
      ...
      console.error('[VibeAgentManager] File system paged list failed:', error);
  ...
  async getSessionMessages(sessionId: string): Promise<ChatMessage[]> {
    try {
      try {
        ...
        console.log(
          '[VibeAgentManager] Session list item for filePath lookup:',
          item,
        );
        if (
          typeof item === 'object' &&
        ...
        console.warn('[VibeAgentManager] JSONL read path lookup failed:', e);
      }
      ...
      console.error(
        '[VibeAgentManager] Failed to get session messages:',
        error,
      );
      return [];
    }
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    try {
      ...
      console.error('[VibeAgentManager] Failed to delete session:', error);
      return false;
    }
  }
  ...
  async renameSession(sessionId: string, title: string): Promise<boolean> {
    try {
      ...
      console.error('[VibeAgentManager] Failed to rename session:', error);
      return false;
    }
  }
  ...
  private async readJsonlMessages(filePath: string): Promise<ChatMessage[]> {
    ...
    try {
      ...
      console.log(
        '[VibeAgentManager] JSONL records read:',
        records.length,
        filePath,
      );
      ...
      console.log(
        '[VibeAgentManager] JSONL messages reconstructed:',
        msgs.length,
      );
      return msgs;
    } catch (err) {
      console.warn('[VibeAgentManager] Failed to read JSONL messages:', err);
      return [];
    }
  }
  ...
  async loadSessionViaAcp(
    ...
    try {
      ...
      console.log(
        '[VibeAgentManager] Rehydration start for session:',
        sessionId,
      );
      console.log(
        '[VibeAgentManager] Attempting session/load via ACP for session:',
        sessionId,
      );
      ...
      console.log(
        '[VibeAgentManager] Session load succeeded. Response:',
      ...
      console.error(
        '[VibeAgentManager] Session load via ACP failed for session:',
        sessionId,
      );
      console.error('[VibeAgentManager] Error type:', error?.constructor?.name);
      console.error('[VibeAgentManager] Error message:', errorMessage);
      ...
      if (error && typeof error === 'object') {
        ...
        if ('error' in error) {
          ...
          if (acpError.error) {
            console.error(
              '[VibeAgentManager] ACP error code:',
              acpError.error.code,
            );
            console.error(
              '[VibeAgentManager] ACP error message:',
              acpError.error.message,
            );
          }
        } else {
          console.error('[VibeAgentManager] Non-ACPIf error details:', error);
        }
      }
      ...
      console.log('[VibeAgentManager] Rehydration end for session:', sessionId);
  ...
  async loadSession(sessionId: string): Promise<ChatMessage[] | null> {
    console.log(
      '[VibeAgentManager] Loading session with version-aware strategy:',
      sessionId,
    );

    try {
      console.log(
        '[VibeAgentManager] Attempting to load session via ACP method',
      );
      await this.loadSessionViaAcp(sessionId);
      console.log('[VibeAgentManager] Session loaded successfully via ACP');
      ...
      console.warn(
        '[VibeAgentManager] ACP session load failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log(
        '[VibeAgentManager] Loading session messages from file system',
      );
      const messages = await this.loadSessionMessagesFromFile(sessionId);
      console.log(
        '[VibeAgentManager] Session messages loaded successfully from file system',
      );
      return messages;
    } catch (error) {
      console.error(
        '[VibeAgentManager] Failed to load session messages from file system:',
        error,
      );
      return null;
    }
  ...
  private async loadSessionMessagesFromFile(
    ...
    try {
      console.log(
        '[VibeAgentManager] Loading session from file system:',
        sessionId,
      );
      ...
      if (!session) {
        console.log(
          '[VibeAgentManager] Session not found in file system:',
          sessionId,
        );
        return null;
      }
      ...
      console.error(
        '[VibeAgentManager] Session load from file system failed:',
        error,
      );
      throw error;
    }
  ...
  async createNewSession(
    ...
    if (!forceNew && this.connection.currentSessionId) {
      console.log(
        '[VibeAgentManager] createNewSession: reusing existing session',
    ...
    if (this.sessionCreateInFlight) {
      console.log(
        '[VibeAgentManager] createNewSession: session creation already in flight',
      );
      if (!forceNew) {
    ...
    console.log('[VibeAgentManager] Creating new session...');

    this.sessionCreateInFlight = (async () => {
      try {
        let newSessionResult: unknown;
        // Try to create a new ACP session. If Vibe asks for auth, let it handle authentication.
        try {
          newSessionResult = await this.connection.newSession(workingDir);
          console.log(
            '[VibeAgentManager] newSession returned:',
          ...
          if (requiresAuth) {
            if (!autoAuthenticate) {
              console.warn(
                '[VibeAgentManager] session/new requires authentication but auto-auth is disabled. Deferring until user logs in.',
              );
              throw err;
            }
            console.warn(
              '[VibeAgentManager] session/new requires authentication. Retrying with authenticate...',
            );
            try {
              // Let CLI handle authentication - it's the single source of truth
              await this.connection.authenticate(authMethod);
              console.log(
                '[VibeAgentManager] createNewSession Authentication successful. Retrying session/new...',
              );
              ...
              console.error(
                '[VibeAgentManager] Re-authentication failed:',
                reauthErr,
              );
              throw reauthErr;
            }
        ...
        console.log(
          '[VibeAgentManager] New session created with ID:',
          newSessionId,
        );
        return newSessionId;
      } finally {
  ...
  async cancelCurrentPrompt(): Promise<void> {
    console.log('[VibeAgentManager] Cancelling current prompt');
