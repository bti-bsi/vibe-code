...
/**
 * Vibe Connection Handler
 *
 * Handles Vibe Agent connection establishment, authentication, and session creation
 */
...
export interface VibeConnectionResult {
...
/**
 * Vibe Connection Handler class
 * Handles connection, authentication, and session initialization
 */
export class VibeConnectionHandler {
  /**
   * Connect to Vibe service and establish session
   *
  ...
  async connect(
    ...
    console.log(`[VibeAgentManager] 🚀 CONNECT() CALLED - ID: ${connectId}`);
    ...
    if (proxyUrl) {
      ...
      console.log(
        '[VibeAgentManager] Using proxy from VSCode settings:',
        proxyUrl,
      );
    }
    ...
    for (let attempt = 1; attempt <= maxConnectAttempts; attempt++) {
      try {
        console.log(
          `[VibeAgentManager] Connecting to ACP process (attempt ${attempt}/${maxConnectAttempts})...`,
        );
        await connection.connect(cliEntryPath!, workingDir, extraArgs);
        console.log('[VibeAgentManager] ACP process connected successfully');
        break;
      } catch (connectError) {
        console.error(
          `[VibeAgentManager] Connect attempt ${attempt} failed:`,
        ...
        console.log(`[VibeAgentManager] Retrying connect in ${delay}ms...`);
    ...
    if (!sessionRestored) {
      console.log(
        '[VibeAgentManager] no sessionRestored, Creating new session...',
      );

      try {
        console.log(
          '[VibeAgentManager] Creating new session (letting CLI handle authentication)...',
        );
        ...
        if (
          ...
          console.log(
            '[VibeAgentManager] Extracted availableModels from session/new:',
        ...
        console.log('[VibeAgentManager] New session created successfully');
        sessionCreated = true;
      } catch (sessionError) {
        const needsAuth =
        ...
        if (needsAuth) {
          requiresAuth = true;
          console.log(
            '[VibeAgentManager] Session creation requires authentication; waiting for user-triggered login.',
          );
        } else {
          console.log(
            `\n⚠️ [SESSION FAILED] newSessionWithRetry threw error\n`,
          );
          console.log(`[VibeAgentManager] Error details:`, sessionError);
          throw sessionError;
        }
      }
    } else {
    ...
    console.log(`\n========================================`);
    console.log(`[VibeAgentManager] ✅ CONNECT() COMPLETED SUCCESSFULLY`);
    console.log(`========================================\n`);
    return {
      sessionCreated,
      requiresAuth,
  ...
  private async newSessionWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `[VibeAgentManager] Creating session (attempt ${attempt}/${maxRetries})...`,
        );
        const res = await connection.newSession(workingDir);
        console.log('[VibeAgentManager] Session created successfully');
        return res;
      } catch (error) {
        lastError = error;
        const errorMessage = getErrorMessage(error);
        console.error(
          `[VibeAgentManager] Session creation attempt ${attempt} failed:`,
          errorMessage,
        );

        // If Vibe reports that authentication is required, try to
        // authenticate on-the-fly once and retry without waiting.
        const requiresAuth = isAuthenticationRequiredError(error);
        if (requiresAuth) {
          if (!autoAuthenticate) {
            console.log(
              '[VibeAgentManager] Authentication required but auto-authentication is disabled. Propagating error.',
            );
            throw error;
          }
          console.log(
            '[VibeAgentManager] Vibe requires authentication. Authenticating and retrying session/new...',
          );
          try {
            ...
            console.log(
              '[VibeAgentManager] newSessionWithRetry Authentication successful',
            );
            // Retry immediately after successful auth
            const res = await connection.newSession(workingDir);
            console.log(
              '[VibeAgentManager] Session created successfully after auth',
            );
            return res;
          } catch (authErr) {
            console.error(
        ...
        console.log(`[VibeAgentManager] Retrying in ${delay}ms...`);
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
