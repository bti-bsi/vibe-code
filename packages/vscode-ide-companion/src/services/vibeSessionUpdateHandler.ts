...
/**
 * Vibe Session Update Handler class
 * Processes various session update events and calls appropriate callbacks
 */
export class VibeSessionUpdateHandler {
  private callbacks: VibeAgentCallbacks;

  constructor(callbacks: VibeAgentCallbacks) {
   ...
   */
  updateCallbacks(callbacks: VibeAgentCallbacks): void {
    this.callbacks = callbacks;
  }
  ...
  handleSessionUpdate(data: SessionNotification): void {
    ...
    switch (sessionUpdate) {
      ...
      default:
        console.log('[VibeAgentManager] Unhandled session update type');
        break;
    }
  }
