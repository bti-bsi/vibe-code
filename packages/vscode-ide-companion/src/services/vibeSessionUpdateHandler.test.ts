...
import type { VibeAgentCallbacks } from '../types/chatTypes.js';

describe('VibeSessionUpdateHandler', () => {
  let handler: VibeSessionUpdateHandler;
  let mockCallbacks: VibeAgentCallbacks;
  ...
  beforeEach(() => {
    ...
    handler = new VibeSessionUpdateHandler(mockCallbacks);
  });
  ...
  describe('plan handling', () => {
    ...
    it('falls back to stream chunk when onPlan is not set', () => {
      const handlerWithStream = new VibeSessionUpdateHandler({
        onStreamChunk: vi.fn(),
      });
  ...
  describe('available_commands_update handling', () => {
    ...
    it('does not call callback when onAvailableCommands is not set', () => {
      const handlerWithoutCallback = new VibeSessionUpdateHandler({});
