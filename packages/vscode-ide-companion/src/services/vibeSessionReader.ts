...
export class VibeSessionReader {
  ...
  async getAllSessions(
    ...
    try {
      ...
      if (!allProjects && workingDir) {
        ...
        const chatsDir = path.join(this.vibeDir, 'tmp', projectHash, 'chats');
        const projectSessions = await this.readSessionsFromDir(chatsDir);
        sessions.push(...projectSessions);
      } else {
        // All projects
        const tmpDir = path.join(this.vibeDir, 'tmp');
        if (!fs.existsSync(tmpDir)) {
          console.log('[VibeSessionReader] Tmp directory not found:', tmpDir);
          return [];
        }
      ...
      console.error('[VibeSessionReader] Failed to get sessions:', error);
      return [];
    }
  }
   ...
   */
  private async readSessionsFromDir(chatsDir: string): Promise<VibeSession[]> {
    const sessions: VibeSession[] = [];
    ...
    for (const file of jsonSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[VibeSessionReader] Failed to read session file:',
          filePath,
          error,
        );
      }
    ...
    for (const file of jsonlSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[VibeSessionReader] Failed to read JSONL session file:',
          filePath,
          error,
        );
      }
   ...
   */
  getSessionTitle(session: VibeSession): string {
  ...
  private async readJsonlSession(
    ...
    try {
      ...
      console.error(
        '[VibeSessionReader] Failed to parse JSONL session:',
        error,
      );
      return null;
    }
  ...
  async deleteSession(sessionId: string, workingDir: string): Promise<boolean> {
    try {
      ...
      console.error('[VibeSessionReader] Failed to delete session:', error);
      return false;
    }
  }
  ...
  async renameSession(
    ...
    try {
      ...
      console.error('[VibeSessionReader] Failed to rename session:', error);
      return false;
    }
  }
}
