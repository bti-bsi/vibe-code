#!/usr/bin/env python3
"""Run real end-to-end smoke tests against vibe CLI using vibe_code_sdk.
...
from vibe_code_sdk import (  # noqa: E402
    SDKUserMessage,
    SyncQuery,
...
from vibe_code_sdk.transport import prepare_spawn_info  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run real vibe_code_sdk smoke tests using vibe CLI",
    )
    parser.add_argument(
        "--vibe",
        default="vibe",
        help="Path or command for vibe executable (default: vibe)",
    )
    parser.add_argument(
        "--cwd",
...
def check_vibe_cli_available(vibe_cmd: str, timeout_seconds: float) -> str:
    spawn_info = prepare_spawn_info(vibe_cmd)
...
def build_options(args: argparse.Namespace) -> dict[str, Any]:
    return {
        "cwd": args.cwd,
        "path_to_vibe_executable": args.vibe,
...
def build_failure_payload(
    *,
    ...
    if vibe_version is not None:
        payload["vibe_version"] = vibe_version
    if completed:
...
async def main() -> int:
    ...
    try:
        vibe_version = check_vibe_cli_available(args.vibe, args.timeout_seconds)
    ...
    try:
        if not args.json_only:
            print(f"[smoke] vibe version: {vibe_version}")
