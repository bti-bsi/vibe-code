...
import pytest
from vibe_code_sdk.transport import build_cli_arguments, prepare_spawn_info
from vibe_code_sdk.types import QueryOptions, TimeoutOptions
...
def test_prepare_spawn_info_uses_runtime_for_python_scripts(tmp_path: Path) -> None:
    script_path = tmp_path / "fake-vibe.py"
...
def test_prepare_spawn_info_uses_node_for_javascript_files(tmp_path: Path) -> None:
    script_path = tmp_path / "fake-vibe.js"
...
def test_prepare_spawn_info_keeps_plain_command_names() -> None:
    spawn_info = prepare_spawn_info("vibe-custom")

    assert spawn_info.command == "vibe-custom"
...
def test_prepare_spawn_info_defaults_to_vibe_when_none() -> None:
    spawn_info = prepare_spawn_info(None)

    assert spawn_info.command == "vibe"
    assert spawn_info.args == []
