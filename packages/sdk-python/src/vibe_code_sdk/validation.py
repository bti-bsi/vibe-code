...
_VALID_AUTH_TYPES = {"openai", "anthropic", "vibe-oauth", "gemini", "vertex-ai"}
...
def validate_query_options(options: QueryOptions) -> None:
    ...
    if options.auth_type and options.auth_type not in _VALID_AUTH_TYPES:
        raise ValidationError(
            ...
            "Expected one of: openai, anthropic, vibe-oauth, gemini, vertex-ai."
        )
    ...
    if (
        options.path_to_vibe_executable is not None
        and not options.path_to_vibe_executable.strip()
    ):
        raise ValidationError("path_to_vibe_executable cannot be empty")
