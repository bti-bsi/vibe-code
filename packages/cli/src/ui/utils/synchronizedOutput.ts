...
export function terminalSupportsSynchronizedOutput(
  ...
  if (
    env['VIBE_CODE_DISABLE_SYNCHRONIZED_OUTPUT'] === '1' ||
    env['VIBE_CODE_SYNCHRONIZED_OUTPUT'] === '0'
  ) {
    return false;
  }

  if (
    env['VIBE_CODE_FORCE_SYNCHRONIZED_OUTPUT'] === '1' ||
    env['VIBE_CODE_SYNCHRONIZED_OUTPUT'] === '1'
  ) {
    return true;
  }
