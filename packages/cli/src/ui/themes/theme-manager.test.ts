...
describe('ThemeManager', () => {
  ...
  describe('auto theme detection', () => {
    it('should select Vibe Dark when terminal is detected as dark', () => {
      ...
      expect(themeManager.getActiveTheme().name).toBe('Vibe Dark');
    });

    it('should select Vibe Light when terminal is detected as light', () => {
      vi.mocked(detectModule.detectTerminalTheme).mockReturnValue('light');
      const result = themeManager.setActiveTheme(AUTO_THEME_NAME);
      expect(result).toBe(true);
      expect(themeManager.getActiveTheme().name).toBe('Vibe Light');
    });
    ...
    it('should resolve async auto theme with Vibe Light for light', async () => {
    ...
    it('should resolve async auto theme with Vibe Dark for dark', async () => {
      vi.mocked(detectModule.detectTerminalThemeAsync).mockResolvedValue(
        'dark',
      );
      await themeManager.resolveAutoThemeAsync();
