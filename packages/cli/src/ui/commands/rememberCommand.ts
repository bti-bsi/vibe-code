...
export const rememberCommand: SlashCommand = {
  ...
  action: (context: CommandContext, args): SlashCommandActionReturn | void => {
    ...
    // Managed auto-memory is disabled: ask the agent to save to VIBE.md
    // using its native file tools. We do not call save_memory because that
    // tool was removed.
    return {
      type: 'submit_prompt',
      content: `Please save the following fact to memory (e.g. append to VIBE.md in the project root):\n\n${fact}`,
    };
  },
};
