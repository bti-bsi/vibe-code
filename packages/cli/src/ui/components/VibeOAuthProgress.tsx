...
interface VibeOAuthProgressProps {
  onTimeout: () => void;
  onCancel: () => void;
...
export function VibeOAuthProgress({
  onTimeout,
  onCancel,
  deviceAuth,
  authStatus,
  authMessage,
}: VibeOAuthProgressProps): React.JSX.Element {
    ...
    return (
      ...
      >
        <Text bold color={theme.status.error}>
          {t('Vibe OAuth Authentication Timeout')}
        </Text>
  ...
  if (authStatus === 'error') {
    return (
      ...
      >
        <Text bold color={theme.status.error}>
          {t('Vibe OAuth Authentication Error')}
        </Text>
  ...
  if (!deviceAuth) {
    return (
      <Box
        ...
        width="100%"
      >
        <Text bold>{t('Vibe OAuth Authentication')}</Text>

        <Box marginTop={1} flexDirection="column">
          <Text>{t('Waiting for Vibe OAuth authentication...')}</Text>
          <Text>
