...
const createMockConfig = (overrides = {}) => ({
  ...
  getTargetDir: vi.fn(() => '/projects/vibe-code'),
  getMcpServers: vi.fn(() => ({})),
...
describe('<AppHeader />', () => {
  it('shows the working directory', () => {
    ...
    expect(lastFrame()).toContain('/projects/vibe-code');
  });
  ...
  it('hides the header when screen reader is enabled', () => {
    ...
    expect(lastFrame()).not.toContain('/projects/vibe-code');
    expect(lastFrame()).not.toContain('Vibe Code');
  });

  it('shows the header with all info when banner is visible', () => {
    const { lastFrame } = renderWithProviders(createMockUIState());
    expect(lastFrame()).toContain('>_ Vibe Code');
