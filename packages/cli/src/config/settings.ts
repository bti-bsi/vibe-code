...
export const SETTINGS_DIRECTORY_NAME = '.vibe';
...
export function getSystemSettingsPath(): string {
  if (process.env['VIBE_CODE_SYSTEM_SETTINGS_PATH']) {
    return process.env['VIBE_CODE_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/VibeCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData.vibe-code\\settings.json';
  } else {
    return '/etc/vibe-code/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['VIBE_CODE_SYSTEM_DEFAULTS_PATH']) {
    return process.env['VIBE_CODE_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(settings: Settings, startDir: string): string | null {
  ...
  const userLevelPaths = new Set([
    ...
    path.normalize(path.join(homeDir, VIBE_DIR, '.env')),
  ]);
  ...
  while (true) {
    // Prefer gemini-specific .env under VIBE_DIR
    const geminiEnvPath = path.join(currentDir, VIBE_DIR, '.env');
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      const homeGeminiEnvPath = path.join(homeDir, VIBE_DIR, '.env');
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(VIBE_DIR);
