 ...
 *
 * Activated by setting VIBE_CODE_PROFILE_STARTUP=1. When enabled, collects
 * high-resolution timestamps at key phases of CLI startup and writes a JSON
 * report to ~/.vibe/startup-perf/ on finalization.
 *
...
export function initStartupProfiler(): void {
  ...
  if (process.env['VIBE_CODE_PROFILE_STARTUP'] !== '1') {
    return;
  }
...
export function finalizeStartupProfile(sessionId?: string): void {
  ...
  try {
    const dir = path.join(os.homedir(), '.vibe', 'startup-perf');
