...
import { IdeClient, AuthType } from '@vibe-bti/vibe-code-core';
 ...
 * For bug reports, removes 'vibe-' or 'vibe-code-' prefixes from sandbox names.
 *
 * @param stripPrefix - Whether to strip 'vibe-' prefix (used for bug reports)
 */
export function getSandboxEnv(stripPrefix = false): string {
  ...
  // For bug reports, remove vibe- prefix
  if (stripPrefix) {
    return sandbox.replace(/^vibe-(?:code-)?/, '');
  }
