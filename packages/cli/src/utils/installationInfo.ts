...
import { createDebugLogger, isGitRepository } from '@vibe-bti/vibe-code-core';
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        childProcess.execSync('brew list -1 | grep -q "^vibe-code$"', {
          stdio: 'ignore',
        });
        return {
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g @vibe-bti/vibe-code@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @vibe-bti/vibe-code@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g @vibe-bti/vibe-code@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @vibe-bti/vibe-code@latest';
    return {
