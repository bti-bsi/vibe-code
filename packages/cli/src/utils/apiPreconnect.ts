...
const DEFAULT_BASE_URLS: Record<string, string> = {
  ...
  'vibe-oauth': 'https://coding.dashscope.aliyuncs.com',
 ...
 *
 * @param authType - Authentication type (openai, vibe-oauth, anthropic, etc.)
...
export function preconnectApi(
  ...
  // Check if disabled
  if (process.env['VIBE_CODE_DISABLE_PRECONNECT'] === '1') {
  ...
  try {
    ...
    fetch(targetUrl, {
      ...
      headers: {
        'User-Agent': 'VibeCode-Preconnect/1.0',
      },
      dispatcher,
    } as RequestInit)
      .then(() => {
