...
import { AuthType, OutputFormat } from '@vibe-bti/vibe-code-core';
...
function createMockConfig(overrides?: Partial<Config>): Config {
  const baseModelsConfig = {
    ...
    getCurrentAuthType: vi.fn().mockReturnValue(AuthType.VIBE_OAUTH),
...
describe('validateNonInterActiveAuth', () => {
  ...
  let originalEnvVibeOauth: string | undefined;
  ...
  beforeEach(() => {
    ...
    originalEnvVibeOauth = process.env['VIBE_OAUTH'];
  ...
  afterEach(() => {
    ...
    if (originalEnvVibeOauth !== undefined) {
      process.env['VIBE_OAUTH'] = originalEnvVibeOauth;
    } else {
      delete process.env['VIBE_OAUTH'];
    }
  ...
  it('exits with error for VIBE_OAUTH (free tier discontinued)', async () => {
    const nonInteractiveConfig = createMockConfig({
      refreshAuth: refreshAuthMock,
      getModelsConfig: vi.fn().mockReturnValue({
        getModel: vi.fn().mockReturnValue('default-model'),
  ...
  it('skips validation if useExternalAuth is true', async () => {
    ...
    expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.VIBE_OAUTH);
  });
  ...
  it('exits if currentAuthType does not match enforcedAuthType', async () => {
    mockSettings.merged.security!.auth!.enforcedType = AuthType.VIBE_OAUTH;
    process.env['OPENAI_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = createMockConfig({
      refreshAuth: refreshAuthMock,
      getModelsConfig: vi.fn().mockReturnValue({
    ...
    expect(mockWriteStderrLine).toHaveBeenCalledWith(
      'The configured auth type is vibe-oauth, but the current auth type is openai. Please re-authenticate with the correct type.',
    );
    expect(processExitSpy).toHaveBeenCalledWith(1);
  });
    ...
    it('emits error result and exits when enforced auth mismatches current auth', async () => {
      ...
      expect(emitResultMock).toHaveBeenCalledWith({
        ...
        errorMessage: expect.stringContaining(
          'The configured auth type is vibe-oauth, but the current auth type is openai.',
        ),
        durationMs: 0,
        apiDurationMs: 0,
        numTurns: 0,
