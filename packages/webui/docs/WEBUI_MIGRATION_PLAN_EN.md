...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@vibe-bti/vibe-code-webui` package
...
```typescript
// @vibe-bti/vibe-code-webui/src/context/PlatformContext.ts
...
```javascript
// @vibe-bti/vibe-code-webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
module.exports = {
  presets: [require('@vibe-bti/vibe-code-webui/tailwind.preset')],
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@vibe-bti/vibe-code-webui/dist/**/*.js',
  ],
};
```
...
```
Developer ──► @vibe-bti/vibe-code-webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@vibe-bti/vibe-code-webui';
// or import { FileIcon } from '@vibe-bti/vibe-code-webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@vibe-bti/vibe-code-webui`
...
│     └── Edit files in @vibe-bti/vibe-code-webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@vibe-bti/vibe-code-webui'            │
...
1. **Single Source of Truth**: All UI components live in `@vibe-bti/vibe-code-webui`
