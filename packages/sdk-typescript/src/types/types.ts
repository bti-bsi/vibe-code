 ...
 * - WebSocket: tcp
 *
 * This interface aligns with MCPServerConfig in @vibe-bti/vibe-code-core.
 */
...
/**
 * Configuration options for creating a query session with the Vibe CLI.
 */
export interface QueryOptions {
   ...
   * This takes precedence over the environment variables `OPENAI_MODEL` and `VIBE_MODEL`
   * @example 'vibe-max', 'vibe-plus', 'vibe-turbo'
   */
  model?: string;
   ...
   * - Command name (no path separators): `'vibe'` -> executes from PATH
   ...
   * - Native binary: `'/path/to/vibe'` -> executes directly
   *
   * Runtime detection:
  ...
  /**
   * Environment variables to pass to the Vibe CLI process.
   * These variables will be merged with the current process environment.
   */
  env?: Record<string, string>;

  /**
   * System prompt configuration for the Vibe CLI session.
   *
   * - `string`: fully overrides the main session system prompt
   * - `{ type: 'preset', preset: 'vibe_code', append?: string }`:
   *   uses Vibe Code's built-in prompt as the base and optionally appends extra
  ...
  /**
   * Custom handler for stderr output from the Vibe CLI process.
   ...
   * - 'vibe-oauth': Use Vibe OAuth authentication
   *
   * Though we support 'vibe-oauth', it's not recommended to use it in the SDK.
   * Because the credentials are stored in `~/.vibe` and may need to refresh periodically.
   */
  authType?: AuthType;
   ...
   * This is equivalent to using the `--resume` flag in the Vibe CLI.
