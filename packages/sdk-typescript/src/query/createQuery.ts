...
export function query({
  ...
  /**
   * The prompt to send to the Vibe Code CLI process.
  ...
  const pathToVibeExecutable = options.pathToVibeExecutable;
...
function validateOptions(options: QueryOptions): SpawnInfo | undefined {
  ...
  try {
    return prepareSpawnInfo(options.pathToVibeExecutable);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Invalid pathToVibeExecutable: ${errorMessage}`);
  }
}
