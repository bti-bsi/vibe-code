...
The Vibe Code Java SDK is a minimum experimental SDK for programmatic access to Vibe Code functionality. It provides a Java interface to interact with the Vibe Code CLI, allowing developers to integrate Vibe Code capabilities into their Java applications.

**Context Information:**

- Current Date: Monday 5 January 2026
- Operating System: darwin
- Working Directory: /Users/weigeng/repos/vibe-code/packages/sdk-java

## Project Details

- **Group ID**: com.alibaba
- **Artifact ID**: vibecode-sdk (as per pom.xml)
...
- **API Layer**: Provides the main entry points through `VibeCodeCli` class with simple static methods for basic usage
- **Session Layer**: Manages communication sessions with the Vibe Code CLI through the `Session` class
...
- `VibeCodeCli`: Main entry point with static methods for simple queries
...
The project includes basic unit tests using JUnit 5. The main test class `VibeCodeCliTest` demonstrates how to use the SDK to make simple queries to the Vibe Code CLI.
...
The `TransportOptions` class allows configuration of how the SDK communicates with the Vibe Code CLI:

- `pathToVibeExecutable`: Path to the Vibe Code CLI executable
...
- **Session creation**: Use `VibeCodeCli.newSession()` to create a new session with custom options
...
- **Thread Naming**: "vibe_code_cli-pool-{number}"
...
The SDK includes several example files in `src/test/java/com/alibaba/vibe/code/cli/example/` that demonstrate different aspects of the API:
...
### Q: Do I need to install the Vibe CLI separately?
