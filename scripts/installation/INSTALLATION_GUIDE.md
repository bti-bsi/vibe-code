# Installation Guide for Vibe Code with Source Tracking

This guide describes how to install Node.js and Vibe Code with source information tracking.

## Overview

The installation scripts automate the process of installing Node.js (if not present or below version 20) and Vibe Code, while capturing and storing the installation source information for analytics and tracking purposes.

## Installation Scripts

We provide platform-specific installation scripts:

- **Linux/macOS**: `install-vibe-with-source.sh`
- **Windows**: `install-vibe-with-source.bat`

## Linux/macOS Installation

### Script: install-vibe-with-source.sh

#### Features:

- Checks for existing Node.js installation and version
- Installs Node.js 20+ if needed using NVM
- Installs Vibe Code globally with source information
- Stores the source information in `~/.vibe/source.json`

#### Usage:

```bash
# Install with a specific source
sh install-vibe-with-source.sh --source github

# Install with internal source
sh install-vibe-with-source.sh -s internal

# Show help
sh install-vibe-with-source.sh --help
```
...
1. The script accepts a `--source` parameter to specify where Vibe Code is being installed from
2. It installs Node.js if needed
3. It installs Vibe Code globally
4. It creates `~/.vibe/source.json` with the specified source information
...
This is required to load the newly installed Node.js and Vibe Code into your PATH.
...
### Script: install-vibe-with-source.bat
...
- Installs Vibe Code globally with source information
- Stores the source information in `%USERPROFILE%\.vibe\source.json`

#### Prerequisites:

- **PowerShell (Administrator)**: The script must be run in PowerShell with Administrator privileges
- Internet connection for downloading Node.js and Vibe Code
...
./install-vibe-with-source.bat --source github

# Install with short parameter
./install-vibe-with-source.bat -s internal

# Use default source (unknown)
./install-vibe-with-source.bat
```
...
1. The script accepts a `--source` or `-s` parameter to specify where Vibe Code is being installed from
...
4. It installs Vibe Code globally using npm
5. It creates `%USERPROFILE%\.vibe\source.json` with the specified source information
...
This feature implements the ability to capture and store the installation source of the Vibe Code package. The source information is used for analytics and tracking purposes.

### Storage Location

The installation source is stored in a separate file at:

- **Unix/Linux/macOS**: `~/.vibe/source.json`
- **Windows**: `%USERPROFILE%\.vibe\source.json` (equivalent to `C:\Users\{username}\.vibe\source.json`)
...
2. **Analytics**: Helps understand how users are discovering and installing Vibe Code
...
- The `VibeLogger` class reads this file during telemetry initialization
...
```bash
cat ~/.vibe/source.json
```

**Windows:**

```cmd
type %USERPROFILE%\.vibe\source.json
```
...
./install-vibe-with-source.bat -s github
```
...
- Ensure NVM is installed: `curl -o- https://vibe-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh | bash`
...
- The scripts require internet access to download Node.js and Vibe Code
