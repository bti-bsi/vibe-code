...
# Vibe Code Installation Script
# This script installs Node.js (via NVM) and Vibe Code CLI
# Supports Linux and macOS
#
# Usage: install-vibe-with-source.sh --source [github|npm|internal|local-build]
#        install-vibe-with-source.sh -s [github|npm|internal|local-build]
...
if [ -z "${BASH_VERSION}" ] && [ -z "${__VIBE_INSTALL_REEXEC:-}" ]; then
    # Check if we're in a git hook environment
    case "${0}" in
        *.git/hooks/*) export __VIBE_IN_GIT_HOOK=1 ;;
    esac
    if [ -n "${GIT_DIR:-}" ]; then
        export __VIBE_IN_GIT_HOOK=1
    fi

    # Try to find bash
    if command -v bash >/dev/null 2>&1; then
        export __VIBE_INSTALL_REEXEC=1
...
# ============================================
echo "=========================================="
echo "   Vibe Code Installation Script"
echo "=========================================="
echo ""
...
install_nvm() {
    ...
    if "${DOWNLOAD_CMD}" "${DOWNLOAD_ARGS}" "https://vibe-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh" > "${NVM_INSTALL_TEMP}"; then
    ...
    # Check if profile file is writable
        ...
        {
            echo ""
            echo "# NVM configuration (added by Vibe Code installer)"
...
fix_npm_permissions() {
    ...
    if [[ "${use_user_dir}" == true ]]; then
        ...
        if [[ -n "${PROFILE_FILE}" ]] && ! grep -q '.npm-global/bin' "${PROFILE_FILE}" 2>/dev/null; then
            {
                echo ""
                echo "# NPM global bin (added by Vibe Code installer)"
...
install_vibe_code() {
    ...
    if command_exists vibe; then
        ...
        log_success "Vibe Code is already installed: ${VIBE_VERSION}"
    ...
    # Install Vibe Code
    log_info "Installing Vibe Code..."
    if npm install -g @vibe-code/vibe-code@latest --registry https://registry.npmmirror.com; then
        log_success "Vibe Code installed successfully!"

        # Verify installation
        if command_exists vibe; then
            local vibe_version
            vibe_version=$(vibe --version 2>/dev/null) || vibe_version="unknown"
            log_info "Vibe Code version: ${vibe_version}"
        fi
    else
        log_error "Failed to install Vibe Code!"
...
create_source_json() {
    ...
    cat > "${VIBE_DIR}/source.json" <<EOF
{
  "source": "${ESCAPED_SOURCE}"
}
EOF

    log_success "Installation source saved to ~/.vibe/source.json"
}
...
main() {
    ...
    # Check if vibe is immediately available
    if command_exists vibe; then
        log_success "Vibe Code is ready to use!"
        echo ""
        echo "You can now run: vibe"
        echo ""
        # Auto-start vibe
        log_info "Starting Vibe Code..."
        echo ""
        exec vibe
    else
        log_warning "Vibe Code command not found in current session"
        echo ""
        echo "To use Vibe Code immediately without restarting your terminal,"
        echo "run the following command in your current shell:"
        echo "  eval \$(${0} --print-env)"
        echo ""
        log_info "Or simply restart your terminal, then run: vibe"
    fi
}
