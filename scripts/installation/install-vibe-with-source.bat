@echo off
REM Script to install Node.js and Vibe Code with source information
REM This script handles the installation process and sets the installation source
REM
REM Usage: install-vibe-with-source.bat --source <source>
REM        install-vibe-with-source.bat -s <source>
REM
...
echo ===========================================
echo Vibe Code Installation Script with Source Tracking
echo ===========================================
echo.
...
if !ERRORLEVEL! EQU 0 (
    ...
    if !MAJOR_VERSION! GEQ 20 (
        ...
        if !ERRORLEVEL! NEQ 0 (
            echo ERROR: Failed to install Node.js. Cannot continue with Vibe Code installation.
            exit /b 1
        )
    )
) else (
...
REM Verify npm is available before installing Vibe Code
...
REM Install Vibe Code with source information
echo INFO: Installing Vibe Code with source: %SOURCE%
echo INFO: Running: %NPM_CMD% install -g @vibe-code/vibe-code@latest --registry https://registry.npmmirror.com
call "%NPM_CMD%" install -g @vibe-code/vibe-code@latest --registry https://registry.npmmirror.com

if %ERRORLEVEL% EQU 0 (
    echo SUCCESS: Vibe Code installed successfully!
) else (
    echo ERROR: Failed to install Vibe Code.
    exit /b 1
)

REM Create source.json only if --source or -s was explicitly provided
if not "!SOURCE!"=="unknown" (
    echo INFO: Creating source.json in %USERPROFILE%\.vibe...

    set "VIBE_DIR=%USERPROFILE%\.vibe"
    ...
    echo   "source": "!SOURCE!"
    echo }
    ) > "!VIBE_DIR!\source.json"

    echo SUCCESS: Installation source saved to %USERPROFILE%\.vibe\source.json
)

REM Verify installation
call :CheckCommandExists vibe
if %ERRORLEVEL% EQU 0 (
    echo SUCCESS: Vibe Code is available as 'vibe' command.
    call vibe --version
    echo.
    echo INFO: Starting Vibe Code...
    echo.
    call vibe
) else (
    echo WARNING: Vibe Code may not be in PATH. Please check your npm global bin directory.
    echo.
    echo ===========================================
    echo SUCCESS: Installation completed!
    echo The source information is stored in %USERPROFILE%\.vibe\source.json
    echo.
    echo ===========================================
)
...
set "TEMP_DIR=%TEMP%\vibe-nodejs-install"
