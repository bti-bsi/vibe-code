...
const USER_GEMINI_DIR = path.join(os.homedir(), '.vibe');
// Project-level .gemini directory in the workspace
const WORKSPACE_VIBE_DIR = path.join(projectRoot, '.vibe');

// Telemetry artifacts are stored in a hashed directory under the user's ~/.vibe/tmp
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `vibe-code-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: vibe-code-dev-script', '-o', tmpFile, url],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'vibe-code-telemetry-'));
