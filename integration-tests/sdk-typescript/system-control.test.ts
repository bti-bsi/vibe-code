  ...
  describe('setModel API', () => {
    it('should change model dynamically during streaming input', async () => {
      ...
      try {
        ...
        await q.setModel('vibe3-vl-plus');
        ...
        // Verify system messages - model should change from vibe3-max to vibe3-vl-plus
        expect(systemMessages.length).toBeGreaterThanOrEqual(2);
        expect(systemMessages[0].model).toBeOneOf(['vibe3-max', 'coder-model']);
        expect(systemMessages[1].model).toBe('vibe3-vl-plus');
      } finally {
        await q.close();
      }
    });
    ...
    it('should handle multiple model changes in sequence', async () => {
      ...
      try {
        ...
        expect(systemMessages[0].model).toBeOneOf(['vibe3-max', 'coder-model']);
        expect(systemMessages[1].model).toBe('vibe3-turbo');
        expect(systemMessages[2].model).toBe('vibe3-vl-plus');
      } finally {
        await q.close();
      }
    });
    ...
    it('should throw error when setModel is called on closed query', async () => {
      ...
      await expect(q.setModel('vibe3-turbo')).rejects.toThrow(
        'Query is closed',
      );
    });
  });
