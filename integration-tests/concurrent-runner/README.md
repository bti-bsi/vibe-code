# Vibe Concurrent Runner

A Python tool for executing multiple Vibe CLI tasks across different models concurrently using isolated git worktrees.
...
```json
{
  ...
  "models": ["claude-3-5-sonnet-20241022", "vibe3-coder-plus"]
}
```
...
| `worktree_base` | string | ~/.vibe/worktrees | Base directory for git worktrees              |
  ...
  "runs": [
    {
      ...
      "status": "succeeded",
      "worktree_path": "~/.vibe/worktrees/run-abc123",
...
4. **Execute**: Run vibe CLI with captured output (logs go to run-specific folder)
