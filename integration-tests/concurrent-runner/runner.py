#!/usr/bin/env python3
"""
Vibe Concurrent Runner - Execute multiple CLI tasks across different models concurrently.

This tool creates isolated git worktrees for each task/model combination and executes
the Vibe CLI in parallel with status tracking and output capture.
"""
...
class RunConfig:
    ...
    worktree_base: Path = field(default_factory=lambda: Path.home() / ".vibe" / "worktrees")
...
class RunRecord:
    ...
    model: str
    status: RunStatus
    auth_type: Optional[str] = None  # e.g. "anthropic" for vibe --auth-type
...
class GitWorktreeManager:
    ...
    async def collect_session_log(self, worktree_dir: Path, output_dir: Path) -> Optional[Tuple[Path, str, Path]]:
        ...
        ~/.vibe/projects/{projectId}/chats/{sessionId}.jsonl
        ...
        chats_dir = vibe_dir / "projects" / project_id / "chats"
...
<head>
    <meta charset="UTF-8">
    <title>Vibe Runner Report</title>
    <style>
...
<body>
    <h1>Vibe Runner Execution Report</h1>
    <div class="summary">
...
class VibeRunner:
    """Executes the Vibe CLI for a specific task and model."""
    ...
    async def run(
        ...
        """Execute the Vibe CLI for each prompt sequentially."""
        ...
        for prompt_index, prompt_text in enumerate(task.prompts, start=1):
            ...
            env["VIBE_CODE_ROOT"] = str(worktree_dir_resolved)
    ...
    def _build_command(self, run: RunRecord, prompt_text: str, use_continue: bool = False) -> List[str]:
        """Build the vibe CLI command for a single prompt."""
        cmd = ["vibe"]
...
def load_config(config_path: Path) -> RunConfig:
    ...
    return RunConfig(
        ...
        worktree_base=Path(data.get("worktree_base", "~/.vibe/worktrees")).expanduser(),
...
async def execute_single_run(
    ...
    try:
        ...
        await vibe_runner.run(run, worktree_dir, output_dir)
...
async def run_all(config: RunConfig, console: Console) -> ExecutionState:
    ...
    vibe_runner = VibeRunner(config, console)
    ...
    async def execute_with_limit(run: RunRecord):
        async with semaphore:
            await execute_single_run(
                run, config, tracker, worktree_manager, vibe_runner, console
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Vibe Concurrent Runner - Execute multiple CLI tasks across models"
    )
    parser.add_argument(
        "config",
        type=Path,
