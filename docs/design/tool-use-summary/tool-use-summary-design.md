...
## 3. Vibe Code Implementation

### 3.1 Flow

Vibe Code hooks the same lifecycle point (`useGeminiStream.handleCompletedTools`) but renders on both sides of `ui.compactMode` so the feature is useful to CLI users without any SDK plumbing.
...
1. `VIBE_CODE_EMIT_TOOL_USE_SUMMARIES=0|1|true|false` — env override, highest priority.
...
| Deviation                                                                | Why                                                                                                                                                                                     |
| ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Settings layer in addition to env gate                                   | Vibe Code renders the label in the CLI; users need a persistent switch, not a per-shell env export.                                                                                     |
...
| Dedicated `cleanSummary` post-processing                                 | Vibe Code supports more heterogeneous providers than CC; some models prepend `Label:` or wrap in quotes. Normalizing at the boundary keeps the UI consistent.                           |
...
| Dual render paths (full-mode inline + compact-mode header)               | Vibe Code's default is `ui.compactMode: false`; without the inline full-mode render, the feature would be invisible to most users.                                                      |
