...
(VIBE.md / managed auto-memory) as a suffix. The final system prompt is
...
| `fastModel`                                | unset   | Recommended (e.g. `vibe3-coder-flash`) for fast and cheap recaps.                   |
...
`~/.vibe/debug/<sessionId>.txt` (`latest.txt` symlinks to the current
session); disable via `VIBE_DEBUG_LOG_FILE=0`.
...
| `VIBE_CODE_ENABLE_AWAY_SUMMARY` env var          | Claude Code uses it to keep the feature on when telemetry is disabled; Vibe Code's current telemetry model doesn't need this.            |
