...
Vibe 现有 command 系统的问题，不是完全没有能力，而是：
...
### 3.2 保留 Vibe 的统一 outcome 模型

不建议机械复制 Claude 的执行实现。

Vibe 当前统一结果模型仍然值得保留，因为它天然适合：
...
Vibe 的 command schema 从第一阶段开始就必须覆盖以下来源：
...
这样 Vibe 才能在体验上接近 Claude 对 `/review`、`/commit`、`/openspec-apply` 这类能力的处理方式。
...
- **用 Vibe 的内部架构风格，交付一个在外部体验上 95% 对齐 Claude Code 的 command 平台**
