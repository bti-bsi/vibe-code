...
用 Vibe 内部架构风格，交付一个在外部体验上 95% 对齐 Claude Code 的 command 平台，同时修复三模式分裂、命令来源单一、prompt command 无法被模型调用三个核心问题。
...
在 Phase 1/2 的元数据和命令能力基础上，补齐补全体验，并补充 Claude Code 中存在而 Vibe Code 缺失的命令。
...
补充 Vibe Code 当前没有、Claude Code 有且常用的命令：
