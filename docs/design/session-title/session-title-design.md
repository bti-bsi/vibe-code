...
1. Cross-provider reliability — OpenAI-compatible endpoints, Gemini, and
   Vibe's native tool-calling all implement function calling; tag parsing
...
symlink planted in `~/.vibe/projects/<proj>/chats/` can't redirect a
...
4. `!config.isInteractive()` → skip. Headless `vibe -p` / CI never spends
   fast-model tokens on a one-shot session.
5. `autoTitleDisabledByEnv()` → skip. `VIBE_DISABLE_AUTO_TITLE=1`
   explicit opt-out.
...
| `VIBE_DISABLE_AUTO_TITLE=1` | unset   | Opt out of the auto trigger without unsetting `fastModel`. `/rename --auto` still works on request. |
...
| Non-interactive auto-titling                | `vibe -p` / CI scripts throw the session away; fast-model tokens for a title no one will ever resume is pure waste.                         |
