...
- **How the CLI Finds the File:** The CLI reads the port from `QWEN_CODE_IDE_SERVER_PORT`, then reads `~/.vibe/ide/<PORT>.lock`. (Legacy fallbacks exist for older extensions; see note below.)
