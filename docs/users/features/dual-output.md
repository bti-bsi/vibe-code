...
Dual Output is a sidecar mode for the interactive TUI: while Vibe Code keeps
...
Embed Vibe Code inside the IDE. The TUI runs in the editor's integrated
...
A CI job runs Vibe Code with a task prompt. The human sees the TUI in the
...
Integration tests spawn Vibe Code headlessly, drive it with `--input-file`
...
At first glance `--json-fd` looks sufficient — the caller spawns Vibe Code
...
```ts
const child = spawn('vibe', ['--json-fd', '3'], {
...
const pty = spawn(
  ...
  [
    '--json-file',
    '/tmp/vibe-events.jsonl',
    '--input-file',
    '/tmp/vibe-input.jsonl',
  ],
  { cols: 120, rows: 40 },
);
```
...
Run Vibe Code with all three channels enabled:

```bash
mkfifo /tmp/vibe-events.jsonl /tmp/vibe-input.jsonl
vibe \
  --json-file /tmp/vibe-events.jsonl \
  --input-file /tmp/vibe-input.jsonl
```

In a second terminal, tail the event stream:

```bash
cat /tmp/vibe-events.jsonl
```

In a third terminal, push a prompt into the running TUI:

```bash
echo '{"type":"submit","text":"Explain this repo"}' >> /tmp/vibe-input.jsonl
```

The prompt appears in the TUI exactly as if the user typed it, and the
streaming response is mirrored on `/tmp/vibe-events.jsonl`.
...
A typical embedding parent process spawns Vibe Code with both channels:

```ts
import { spawn } from 'node:child_process';
import { openSync } from 'node:fs';

const eventsFd = openSync('/tmp/vibe-events.jsonl', 'w');
const child = spawn(
  'vibe',
  ['--json-fd', '3', '--input-file', '/tmp/vibe-input.jsonl'],
  { stdio: ['inherit', 'inherit', 'inherit', eventsFd] },
);
```
...
appending JSONL lines to `/tmp/vibe-input.jsonl`.
...
```jsonc
// ~/.vibe/settings.json  (user-level)
// or <workspace>/.vibe/settings.json  (workspace-level)
{
  "dualOutput": {
    "jsonFile": "/tmp/vibe-events.jsonl",
    "inputFile": "/tmp/vibe-input.jsonl",
  },
}
```
...
next Vibe Code launch, since the bridge is constructed once during
startup.
...
```bash
# Terminal A
mkfifo /tmp/vibe-events.jsonl
cat /tmp/vibe-events.jsonl | jq -c 'select(.type != "stream_event") | {type, subtype}'

# Terminal B
vibe --json-file /tmp/vibe-events.jsonl
...
```bash
# Terminal A
touch /tmp/vibe-in.jsonl
vibe --input-file /tmp/vibe-in.jsonl
...
mkfifo /tmp/vibe-out.jsonl
touch /tmp/vibe-in.jsonl
(cat /tmp/vibe-out.jsonl \
  | jq -c 'select(.type == "control_request")') &

# Terminal B
vibe --json-file /tmp/vibe-out.jsonl --input-file /tmp/vibe-in.jsonl
# Ask Vibe to do something that needs approval, e.g.
...
The most realistic shape: a parent process spawns Vibe Code, tails
...
const events = join(tmpdir(), `vibe-events-${process.pid}.jsonl`);
const input = join(tmpdir(), `vibe-input-${process.pid}.jsonl`);
writeFileSync(events, '');
writeFileSync(input, '');

const child = spawn('vibe', ['--json-file', events, '--input-file', input], {
  stdio: 'inherit',
});
...
# Vibe Code TUI opens in the current terminal; the embedder logs
# handshake + turn-end + session_end events to the parent's stdout.
```

### POC 5 — capability handshake feature detection

Older Vibe Code versions won't emit `protocol_version`. Treat the field
...
rl.on('line', (line) => {
  ...
  if (ev.type === 'system' && ev.subtype === 'session_start') {
    ...
    if (v < 1) {
      console.error(
        'vibe-code dual output is present but protocol < 1; ' +
          'falling back to best-effort behavior',
      );
    } else {
      console.log('vibe-code dual output protocol v' + v);
    }
  }
});
```
