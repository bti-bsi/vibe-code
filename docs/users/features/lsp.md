...
Vibe Code provides native Language Server Protocol (LSP) support, enabling advanced code intelligence features like go-to-definition, find references, diagnostics, and code actions. This integration allows the AI agent to understand your code more deeply and provide more accurate assistance.

## Overview

LSP support in Vibe Code works by connecting to language servers that understand your code. Once you configure servers via `.lsp.json` (or extensions), Vibe Code can start them and use them to:
...
LSP is an experimental feature in Vibe Code. To enable it, use the `--experimental-lsp` command line flag:

```bash
vibe --experimental-lsp
```

LSP servers are configuration-driven. You must define them in `.lsp.json` (or via extensions) for Vibe Code to start them.
...
Vibe Code exposes LSP functionality through the unified `lsp` tool. Here are the available operations:
...
1. **Verify `--experimental-lsp` flag**: Make sure you're using the flag when starting Vibe Code
...
Extensions can provide LSP server configurations through the `lspServers` field in their `plugin.json`. This can be either an inline object or a path to a `.lsp.json` file. Vibe Code loads these configs when the extension is enabled. The format is the same language-keyed layout used in project `.lsp.json` files.
...
Use the `--experimental-lsp` flag when starting Vibe Code:
