...
Learn how to enable and setup OpenTelemetry for Vibe Code.
...
observability framework — Vibe Code's observability system provides:
...
All telemetry behavior is controlled through your `.vibe/settings.json` file.
These settings can be overridden by environment variables or CLI flags.

| Setting               | Environment Variable                   | CLI Flag                                                 | Description                                          | Values            | Default                 |
| --------------------- | -------------------------------------- | -------------------------------------------------------- | ---------------------------------------------------- | ----------------- | ----------------------- |
| `enabled`             | `VIBE_TELEMETRY_ENABLED`               | `--telemetry` / `--no-telemetry`                         | Enable or disable telemetry                          | `true`/`false`    | `false`                 |
| `target`              | `VIBE_TELEMETRY_TARGET`                | `--telemetry-target <local\|gcp>`                        | Where to send telemetry data                         | `"gcp"`/`"local"` | `"local"`               |
| `otlpEndpoint`        | `VIBE_TELEMETRY_OTLP_ENDPOINT`         | `--telemetry-otlp-endpoint <URL>`                        | OTLP collector endpoint                              | URL string        | `http://localhost:4317` |
| `otlpProtocol`        | `VIBE_TELEMETRY_OTLP_PROTOCOL`         | `--telemetry-otlp-protocol <grpc\|http>`                 | OTLP transport protocol                              | `"grpc"`/`"http"` | `"grpc"`                |
| `otlpTracesEndpoint`  | `VIBE_TELEMETRY_OTLP_TRACES_ENDPOINT`  | -                                                        | Per-signal endpoint override for traces (HTTP only)  | URL string        | -                       |
| `otlpLogsEndpoint`    | `VIBE_TELEMETRY_OTLP_LOGS_ENDPOINT`    | -                                                        | Per-signal endpoint override for logs (HTTP only)    | URL string        | -                       |
| `otlpMetricsEndpoint` | `VIBE_TELEMETRY_OTLP_METRICS_ENDPOINT` | -                                                        | Per-signal endpoint override for metrics (HTTP only) | URL string        | -                       |
| `outfile`             | `VIBE_TELEMETRY_OUTFILE`               | `--telemetry-outfile <path>`                             | Save telemetry to file (overrides `otlpEndpoint`)    | file path         | -                       |
| `logPrompts`          | `VIBE_TELEMETRY_LOG_PROMPTS`           | `--telemetry-log-prompts` / `--no-telemetry-log-prompts` | Include prompts in telemetry logs                    | `true`/`false`    | `true`                  |
| `useCollector`        | `VIBE_TELEMETRY_USE_COLLECTOR`         | -                                                        | Use external OTLP collector (advanced)               | `true`/`false`    | `false`                 |
...
Vibe Code automatically appends signal-specific paths (`/v1/traces`, `/v1/logs`,
...
The `VIBE_TELEMETRY_OTLP_*` variants take precedence over the `OTEL_*` variants.
...
To view Vibe Code telemetry in Alibaba Cloud Managed Service for
OpenTelemetry, configure Vibe Code to export to the OTLP endpoint
provided by ARMS.

Setting `"target": "gcp"` alone does not configure the export
destination. If `otlpEndpoint` is not set, Vibe Code still defaults to
...
1. Enable telemetry in your `.vibe/settings.json` and set the OTLP
   endpoint:
   ...
   **Option B: HTTP protocol with per-signal endpoints** (for backends
   ...
   > per-signal overrides), Vibe Code appends standard OTLP paths
...
2. If your Alibaba Cloud endpoint requires authentication, provide OTLP
   ...
   `OTEL_EXPORTER_OTLP_HEADERS` (or the signal-specific variants). Vibe
   Code does not currently expose OTLP auth headers directly in
   `.vibe/settings.json`.
3. Run Vibe Code and send prompts.
...
1. Enable telemetry in your `.vibe/settings.json`:
   ...
   ```
2. Run Vibe Code and send prompts.
...
1. Run the automation script:
   ...
   - Save logs/metrics to `~/.vibe/tmp/<projectHash>/otel/collector.log`
...
Logs are timestamped records of specific events. The following events are logged for Vibe Code:

- `vibe-code.config`: This event occurs once at startup with the CLI's configuration.
  - **Attributes**:
    - `model` (string)
...
- `vibe-code.user_prompt`: This event occurs when a user submits a prompt.
  - **Attributes**:
    - `prompt_length` (int)
    - `prompt_id` (string)
...
- `vibe-code.tool_call`: This event occurs for each function call.
  - **Attributes**:
    - `function_name`
    - `function_args`
    - `duration_ms`
...
- `vibe-code.file_operation`: This event occurs for each file operation.
  - **Attributes**:
    - `tool_name` (string)
...
- `vibe-code.api_request`: This event occurs when making a request to Vibe API.
  - **Attributes**:
    - `model`
    - `request_text` (if applicable)

- `vibe-code.api_error`: This event occurs if the API request fails.
  - **Attributes**:
    - `model`
    - `error`
    - `error_type`
...
- `vibe-code.api_response`: This event occurs upon receiving a response from Vibe API.
  - **Attributes**:
    - `model`
    - `status_code`
    - `duration_ms`
...
- `vibe-code.tool_output_truncated`: This event occurs when the output of a tool call is too large and gets truncated.
  - **Attributes**:
    - `tool_name` (string)
...
- `vibe-code.malformed_json_response`: This event occurs when a `generateJson` response from Vibe API cannot be parsed as a json.
  - **Attributes**:
    - `model`

- `vibe-code.flash_fallback`: This event occurs when Vibe Code switches to flash as fallback.
  - **Attributes**:
    - `auth_type`

- `vibe-code.slash_command`: This event occurs when a user executes a slash command.
  - **Attributes**:
    - `command` (string)
    - `subcommand` (string, if applicable)

- `vibe-code.extension_enable`: This event occurs when an extension is enabled
- `vibe-code.extension_install`: This event occurs when an extension is installed
  - **Attributes**:
    - `extension_name` (string)
    - `extension_version` (string)
    - `extension_source` (string)
    - `status` (string)
- `vibe-code.extension_uninstall`: This event occurs when an extension is uninstalled

### Metrics

Metrics are numerical measurements of behavior over time. The following metrics are collected for Vibe Code (metric names remain `vibe-code.*` for compatibility):

- `vibe-code.session.count` (Counter, Int): Incremented once per CLI startup.

- `vibe-code.tool.call.count` (Counter, Int): Counts tool calls.
  - **Attributes**:
    - `function_name`
    - `success` (boolean)
...
- `vibe-code.tool.call.latency` (Histogram, ms): Measures tool call latency.
  - **Attributes**:
    - `function_name`
    - `decision` (string: "accept", "reject", or "modify", if applicable)

- `vibe-code.api.request.count` (Counter, Int): Counts all API requests.
  - **Attributes**:
    - `model`
    - `status_code`
    - `error_type` (if applicable)

- `vibe-code.api.request.latency` (Histogram, ms): Measures API request latency.
  - **Attributes**:
    - `model`

- `vibe-code.token.usage` (Counter, Int): Counts the number of tokens used.
  - **Attributes**:
    - `model`
    - `type` (string: "input", "output", "thought", or "cache")

- `vibe-code.file.operation.count` (Counter, Int): Counts file operations.
  - **Attributes**:
...
- `vibe-code.chat_compression` (Counter, Int): Counts chat compression operations
  - **Attributes**:
