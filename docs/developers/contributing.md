...
[Sandboxing](#sandboxing) is highly recommended and requires, at a minimum, setting `VIBE_SANDBOX=true` in your `~/.env` and ensuring a sandboxing provider (e.g. `macOS Seatbelt`, `docker`, or `podman`) is available. See [Sandboxing](#sandboxing) for details.

To build both the `vibe-code` CLI utility and the sandbox container, run `build:all` from the root directory:
...
To start the Vibe Code application from the source code (after building), run the following command from the root directory:

```bash
npm start
```

If you'd like to run the source build outside of the vibe-code folder, you can utilize `npm link path/to/vibe-code/packages/cli` (see: [docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) to run with `vibe-code`
...
The integration tests are designed to validate the end-to-end functionality of Vibe Code. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for Vibe Code.
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect vibe-code due to automatic exclusion. Use `.vibe-code/.env` files for vibe-code specific debug settings.

### React DevTools

To debug the CLI's React-based UI, you can use React DevTools. Ink, the library used for the CLI's interface, is compatible with React DevTools version 4.x.

1.  **Start the Vibe Code application in development mode:**
