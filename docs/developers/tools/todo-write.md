...
This document describes the `todo_write` tool for Vibe Code.
...
## How to use `todo_write` with Vibe Code
...
The tool stores todo lists in your home directory (`~/.vibe/todos/`) with session-specific files, so each coding session maintains its own task list.
