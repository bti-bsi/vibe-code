# MCP servers with Vibe Code

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with Vibe Code.
...
Vibe Code integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
4. **Sanitizes and validates** tool schemas for compatibility with the Vibe API
...
Vibe Code uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
Vibe Code supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
```bash
vibe mcp add --transport sse remote-server https://api.example.com/sse/ \
...
- **`redirectUri`** (string): Custom redirect URI. **Critical for remote deployments**: Defaults to `http://localhost:7777/oauth/callback`. When running Vibe Code on remote/cloud servers, set this to a publicly accessible URL (e.g., `https://your-server.com/oauth/callback`). Can be configured via `vibe mcp add --oauth-redirect-uri` or directly in settings.json.
...
When Vibe Code starts, it performs MCP server discovery through the following detailed process:
...
4. **Name sanitization:** Tool names are cleaned to meet Vibe API requirements:
...
Once discovered, MCP tools are available to the Vibe model like built-in tools. The model will automatically:
...
When Vibe Code receives this response, it will:
...
This enables you to build sophisticated tools that can provide rich, multi-modal context to the Vibe model.

## MCP Prompts as Slash Commands

In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within Vibe Code. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="Vibe Code" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Vibe Code" reverent
```
...
## Managing MCP Servers with `vibe mcp`
...
### Adding a Server (`vibe mcp add`)

The `add` command configures a new MCP server in your `settings.json`. Based on the scope (`-s, --scope`), it will be added to either the user config `~/.vibe/settings.json` or the project config `.vibe/settings.json` file.

**Command:**

```bash
vibe mcp add [options] <name> <commandOrUrl> [args...]
```
...
- `--oauth-redirect-uri`: OAuth redirect URI (e.g., `https://your-server.com/oauth/callback`). Defaults to `http://localhost:7777/oauth/callback` for local setups. **Important for remote deployments**: When running Vibe Code on remote/cloud servers, set this to a publicly accessible URL.
...
```bash
# Basic syntax
vibe mcp add <name> <command> [args...]

# Example: Adding a local server
vibe mcp add my-stdio-server -e API_KEY=123 /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
vibe mcp add python-server python server.py --port 8080
```
...
```bash
# Basic syntax
vibe mcp add --transport http <name> <url>

# Example: Adding an HTTP server
vibe mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
vibe mcp add --transport http secure-http https://api.example.com/mcp/ --header "Authorization: Bearer abc123"
```
...
```bash
# Basic syntax
vibe mcp add --transport sse <name> <url>

# Example: Adding an SSE server
vibe mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
vibe mcp add --transport sse secure-sse https://api.example.com/sse/ --header "Authorization: Bearer abc123"

# Example: Adding an OAuth-enabled SSE server
vibe mcp add --transport sse oauth-server https://api.example.com/sse/ \
...
### Managing Servers (`vibe mcp`)

To view and manage all MCP servers currently configured, use the `manage` command or simply `vibe mcp`. This opens an interactive TUI dialog where you can:
...
### Removing a Server (`vibe mcp remove`)
