...
This document describes the `task` tool for Vibe Code.
...
## How to use `task` with Vibe Code
...
You can view available subagents by using the `/agents` command in Vibe Code.
...
Subagents are configured through Vibe Code's agent configuration system. Use the `/agents` command to:
